...
void LED_Logic(void) {
    ...
    //LED4  = (((IN1) || wake_up1) && FLASH);   //左转向灯
    //LED5  = (VCU_Gear==1); //前进挡指示
    LED6  = (Charge_Status == 2); //充电连接   
    LED7  = (Charge_Status == 1); //充电状态  
    //LED8 = (((IN5) || wake_up1) && FLASH);   //右转向灯
    LED9  = IN31;  //气压1低信号
    LED10 = 1; //总电源指示   
    LED11 = IN19;//气压2低信号
    LED12 = (BMS_SOC * 0.4 < 10 || BMS[1].bit5 || BMS[1].bit8); //SOC低
    LED13 = (Moto_Fault > 0); //电机故障
    LED14 = (BAT_Fault > 0); //动力电池系统故障
    LED15 = IN4; //小灯
    LED16 = (ADV[0] < 180); //蓄电池充电
    LED17 = (IN2); //远光灯
    LED18 = (IN20); //手刹
    LED19 = (IN9); //前雾灯
    LED20 = (IN10); //后雾灯
...
void PCAN_METER21(void) {
    ...
    message_METER21.data[5] = (unsigned char) (mKH1 << 4)//前门开
            ...
            + (unsigned char) ((ADV[0] < 160) << 1)//低压蓄电池低
            + (unsigned char) (0); //后门开
...
void PCAN_MET_S_1(void) {
    ...
    message_MET_S_1.data[0] = (unsigned char) (1 << 6)//ON档
            ...
            + (unsigned char) (rKL8 << 1)//驻车信号
            + (unsigned char) ((ADR[3] < 2000) || (ADR[4] < 2000)); //气压低报警信号
...
void PCAN_send_1(void) {
    ...
    msg.data[3] = (unsigned char) ((total_miles / 10) >> 8);//中字节
    msg.data[4] = (unsigned char) (total_miles / 10);//低字节    
    msg.data[5] = (unsigned char) ((total_miles / 10) >> 16);//高字节
    msg.data[6] = 100;
    msg.data[7] = VERISON;
    msg.id = 0x1800D028;
    msg.len = 0x08;
...
unsigned char READY; //整车控制器状态
unsigned char Inverter_Fault; //整车控制器故障
unsigned char Vacuum_Fault; //真空度故障
unsigned char BAT24V_Fault; //蓄电池故障
unsigned char DCDC_Fault; //DCDC故障
unsigned char Speed_Percent; //加速踏板开度
unsigned char Brake_Percent; //制动踏板开度
unsigned int Moto_Torque = 30000; //转矩
unsigned char Motor_Temperature = 40; //电机温度
DATA_BIT VCU[4];

unsigned char VCU_Gear; //档位
unsigned char PTC_Front; //前模块PTC档位
unsigned char PTC_Rear;  //后模块PTC档位
unsigned char Charge_Status; //充电状态
unsigned char BUS_Status; //整车状态
unsigned char Moto_Fault; //电机系统故障
unsigned char BAT_Fault; //电机系统故障
unsigned char VCU_Fault; //整车控制器故障
unsigned char WaterPump_Fault; //水泵故障
unsigned char Fan_Fault; //风扇故障
unsigned char AirPump_Fault; //气泵故障
unsigned char BAT24V_Status; //蓄电池状态
unsigned char Gear_Fault; //挂挡失败
unsigned char Inverter_t = 40; //控制器温度
unsigned char Vacuum; //真空度
//unsigned char BMS_SOC; //电池SOC

unsigned char VCU_Speed; //车速
unsigned int Rest_Mile; //续航里程
unsigned int Moto_Rpm = 15000; //电机转速

unsigned int Moto_Voltage; //电机直流电压
unsigned int Moto_Current = 5000; //电机相电流
unsigned int Moto_PhaseVoltage; //电机相电压
unsigned char Press1; //气压1
unsigned char Press2; //气压2

unsigned long Program; //程序版本

//BMS
unsigned int BMS_V; //电池电压
unsigned int BMS_A = 5000; //电池电流
unsigned char BMS_SOC; //电池SOC
unsigned char BMS_Life; //BMSlife
unsigned char BMS_Fault; //BMS故障等级
unsigned char Oum_Warn; //整车绝缘报警

unsigned char BAT_Voltage_Number[4]; //单体电压序号
unsigned char BAT_Temperature_Number; //单体温度序号
unsigned char BAT_Cell_Voltage[210]; //电池单体电压
unsigned char BAT_Cell_Temperature[60][3]; //电池单体温度

DATA_BIT BMS[8];
unsigned char BAT_Number; //电池组总串数
unsigned int BAT_Ahr; //电池组容量
...
//充电机
unsigned char Charger_Life; //充电机LIFE
unsigned int Charger_Voltage; //充电机输出电压
unsigned int Charger_Current = 5000; //充电机输出电流
DATA_BIT CHARGER[1];
