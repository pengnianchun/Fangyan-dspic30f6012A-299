...
void mainFrame(void) {
    ...
    LCD_DrawRect(0, 430, 800, 2, WHITE); //底部横线
    ...
    */
    ...
    LCD_DrawImage1BPP(225 + 83 + 83 + 2 + 43, 365 - 50 + 10, BMP_Zhuanju, WHITE, BLACK); //转矩
...
void disp_baojing(void) {
    ...
    if (color_old[1] != color) {
        color_old[1] = color;
        LCD_DrawImage1BPP(6 + 66, 10, Bmp_baojing[5], color, BLACK); //门3
    }
    ...
    if (color_old[3] != color) {
        color_old[3] = color;
        LCD_DrawImage1BPP(6 + 66 * 3, 10, Bmp_ECO, color, BLACK); //ECO模式
    }
    ...
    if (color_old[4] != color) {
        color_old[4] = color;
        LCD_DrawImage1BPP(6 + 66 * 4, 10, Bmp_baojing[4], color, BLACK); //安全门
    }
    ...
    if (color_old[11] != color) {
        color_old[11] = color;
        LCD_DrawImage1BPP(6 + 66 * 11, 10, Bmp_baojing[17], color, BLACK); //安全带
    }
    ...
    if (color_old[22] != color) {
        color_old[22] = color;
        //LCD_DrawImage1BPP(6, 10 + 64, Bmp_baojing[16], color, BLACK);//燃油指示
    }
    ...
    if (color_old[15] != color) {
        color_old[15] = color;
        LCD_DrawImage1BPP(138 + 66 * 1, 74, Bmp_baojing[10], color, BLACK); //应急阀
    }
    ...
    if (color_old[16] != color) {
        color_old[16] = color;
        LCD_DrawImage1BPP(138 + 66 * 2, 74, Bmp_baojing[11], color, BLACK); //电机过热
    }
    ...
    if (color_old[18] != color) {
        color_old[18] = color;
        LCD_DrawImage1BPP(138 + 66 * 4, 74, Bmp_baojing[12], color, BLACK); //水温
    }
    ...
    if (color_old[19] != color) {
        color_old[19] = color;
        LCD_DrawImage1BPP(138 + 66 * 5, 74, Bmp_baojing[13], color, BLACK); //乘客门气压
    }
    ...
    if (color_old[20] != color) {
        color_old[20] = color;
        LCD_DrawImage1BPP(138 + 66 * 7, 74, Bmp_baojing[18], color, BLACK); //AC空调
    }
    ...
    if (color_old[21] != color) {
        color_old[21] = color;
        LCD_DrawImage1BPP(138 + 66 * 8, 74, Bmp_baojing[19], color, BLACK); //再生制动
    }
    ...
    if (alarm_update_flag == 0) { //3S 更新一次 
        ...
        switch (al_index) {
            ...
            case 3:
                ...
                al_index = 5;
                if (Vacuum_Fault) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_Zhenkongdu, YELLOW, BLACK); //真空度故障                   
                    LCD_DrawImage1BPP(x + 63, y + 8, BMP_Guzhang, YELLOW, BLACK);
                    LCD_DrawRect(x + 63 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                ...
                if (BAT24V_Fault || BAT24V_Status) {                  
                    LCD_DrawImage1BPP(x, y + 8, BMP_Xu, YELLOW, BLACK); //蓄电池欠压   
                ...
                al_index = 7;
                if (DCDC_Fault) {                  
                    LCD_DrawImage1BPP(x, y + 8, BMP_DCDC, YELLOW, BLACK); //DCDC故障   
                ...
                al_index = 11;
                if (WaterPump_Fault) {                  
                    LCD_DrawImage1BPP(x, y + 8, BMP_Shuibeng, YELLOW, BLACK); //水泵故障
                    LCD_DrawImage1BPP(x + 42 , y + 8, BMP_Guzhang, YELLOW, BLACK);
                    LCD_DrawRect(x + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                ...
                al_index = 12;
                if (Fan_Fault) {                  
                    LCD_DrawImage1BPP(x, y + 8, BMP_Fengshan, YELLOW, BLACK); //风扇故障
                    LCD_DrawImage1BPP(x + 42 , y + 8, BMP_Guzhang, YELLOW, BLACK);
                    LCD_DrawRect(x + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                ...
                al_index = 13;
                if (AirPump_Fault) {                  
                    LCD_DrawImage1BPP(x, y + 8, BMP_Qibeng, YELLOW, BLACK); //气泵故障
                    LCD_DrawImage1BPP(x + 42 , y + 8, BMP_Guzhang, YELLOW, BLACK);
                    LCD_DrawRect(x + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                ...
                al_index = 14;
                if (Gear_Fault) {                  
                    LCD_DrawImage1BPP(x, y + 8, BMP_Guadang, YELLOW, BLACK); //挂挡失败
                    LCD_DrawRect(x + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                }
                ...
                if (VCU[1].byte || VCU[2].byte || VCU[3].byte) {                  
                    LCD_DrawImage1BPP(x, y + 8, BMP_DJ, YELLOW, BLACK); //电机故障保护
                    LCD_DrawImage1BPP(x + 42, y + 8, BMP_Guzhang, YELLOW, BLACK);
                ...
                if (BMS[1].byte || BMS[0].byte) {                  
                    LCD_DrawImage1BPP(x, y + 8, BMP_DC, YELLOW, BLACK); //电池故障
                    LCD_DrawImage1BPP(x + 42, y + 8, BMP_Guzhang, YELLOW, BLACK);
                    LCD_DrawRect(x + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                ...
                if (((CHARGER[0].byte >> 5) & 0x03) > 0) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_Chongdianqiang, YELLOW, BLACK); //充电机故障
                ...
                al_index = 18;
                if (BMS[2].bit1) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警器1故障
                ...
                al_index = 19;
                if (BMS[2].bit2) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警器2故障
                ...
                al_index = 20;
                if (BMS[2].bit3) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警器3故障
                ...
                al_index = 21;
                if (BMS[2].bit4) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警器4故障
                ...
                al_index = 22;
                if (BMS[2].bit5) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警器5故障
                ...
                al_index = 23;
                if (BMS[2].bit6) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警器6故障
                ...
                al_index = 24;
                if (BMS[3].bit1) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警1
                ...
                al_index = 25;
                if (BMS[3].bit2) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警2
                    LCD_DrawRect(x + 84, y, 21, 20, BLACK);
                ...
                al_index = 26;
                if (BMS[3].bit3) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警3
                    LCD_DrawRect(x + 84, y, 21, 20, BLACK);
                ...
                al_index = 27;
                if (BMS[3].bit4) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警4
                    LCD_DrawRect(x + 84, y, 21, 20, BLACK);
                ...
                al_index = 28;
                if (BMS[3].bit5) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警5
                    LCD_DrawRect(x + 84, y, 21, 20, BLACK);
                ...
                al_index = 29;
                if (BMS[3].bit6) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_YWBJQ, YELLOW, BLACK); //烟雾报警6
                    LCD_DrawRect(x + 84, y, 21, 20, BLACK);
                ...
                al_index = 30;
                if (BMS[4].bit1) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_Zong, YELLOW, BLACK); //总负接触器粘连故障
...
void frmMain(void) {
    ...
    if (f_index == 0) { //只更新一次
        ...
        f_index = 2;
        print_SZ_1(60, 318 + 38, BAT_Max_Cell_Voltage * 2, 4, 3, WHITE); //单体高电压
        print_SZ_1(60, 318 + 38 * 2, BAT_Min_Cell_Voltage * 2, 4, 3, WHITE); //单体低电压
        updata_DAT(313, 318 + 38, Motor_Temperature, 1, 40, WHITE, 3, 0); //电机温度
    } else if (f_index == 2) {
        f_index = 3;
        disp_baojing();
        updata_DAT(480 + 30, 318, Moto_Torque, 0.1, 3000, WHITE, 4, 0); //电机转矩
        /*if((BUS_Status == 0) && (status_old != 0)){
            LCD_DrawImage1BPP(480 + 30 + 48, 318 + 38, BMP_Tingche, YELLOW, BLACK); //停车
        }else if((BUS_Status == 1) && (status_old != 1)){
            LCD_DrawImage1BPP(480 + 30 + 48, 318 + 38, BMP_Chong, YELLOW, BLACK); //充电
            LCD_DrawImage1BPP(480 + 30 + 48, 318 + 38, BMP_DC, YELLOW, BLACK); //充电
        }*/
        //status_old = BUS_Status;
        print_SZ_1(480 + 30 + 48, 318 + 38, BUS_Status, 1, 0, WHITE); //整车模式
    } else if (f_index == 3) {
        f_index = 4;
        disp_miles();
...
void MOTOFrame(unsigned char index) {
    ...
    LCD_DrawImage1BPP(82 + 42, 100, BMP_Zhiliu, WHITE, BLACK); //输入
    ...
    LCD_DrawImage1BPP(82 + 42, 100 + 38, BMP_Xiang, WHITE, BLACK); //相
    ...
    LCD_DrawImage1BPP(82 + 42, 100 + 38 * 2, BMP_Xiang, WHITE, BLACK); //相
    ...
    LCD_DrawImage1BPP(82 + 42 + 63, 100 + 38 * 4, BMP_Wendu, WHITE, BLACK); //温度
    ...
    LCD_DrawImage1BPP(82 + 42 + 42 + 320, 100, BMP_Kaidu, WHITE, BLACK); //开度
    ...
    LCD_DrawImage1BPP(82 + 42 + 42 + 320, 100 + 38, BMP_Kaidu, WHITE, BLACK); //开度
    LCD_DrawImage1BPP(720 - 24, 100 + 38, BMP_Baifenhao, WHITE, BLACK); //%

    LCD_DrawImage1BPP(82 + 320, 100 + 38 * 2, BMP_KZQ, WHITE, BLACK); //控制转矩
    ...
    LCD_DrawImage1BPP(82 + 320, 100 + 38 * 3, BMP_Zhenkongdu, WHITE, BLACK); //真空度
    ...
    LCD_DrawImage1BPP(82 + 320, 100 + 38 * 4, BMP_Chesu, WHITE, BLACK); //车速
    ...
    LCD_DrawImage1BPP(82 + 320, 100 + 38 * 5, BMP_Xuhanglicheng, WHITE, BLACK); //续航里程
    LCD_DrawImage1BPP(720 - 30, 100 + 38 * 5, BMP_k, WHITE, BLACK); //KM
    LCD_DrawImage1BPP(720 - 30 + 11, 100 + 38 * 5, BMP_m, WHITE, BLACK); //

    LCD_DrawImage1BPP(82 + 320, 100 + 38 * 6, BMP_Zhuansu, WHITE, BLACK); //转速
    LCD_DrawImage1BPP(720 - 32 - 11, 100 + 38 * 6, BMP_RPM, WHITE, BLACK); //RPM

    LCD_DrawImage1BPP(82 + 320, 100 + 38 * 7, BMP_Qiya, WHITE, BLACK); //气压
    ...
    LCD_DrawImage1BPP(82 + 320, 100 + 38 * 8, BMP_Qiya, WHITE, BLACK); //气压
    ...
    LCD_DrawImage1BPP(82 + 320, 100 + 38 * 9, BMP_Zhengche, WHITE, BLACK); //整车
    LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 9, BMP_Mode, WHITE, BLACK); //模式
    }
    else if (index == 2) {
        ...
        LCD_DrawImage1BPP(82, 100, BMP_Shuibeng, WHITE, BLACK); //水泵
        
        LCD_DrawImage1BPP(82, 100 + 38, BMP_Chesu, WHITE, BLACK); //低速风扇
        LCD_DrawImage1BPP(82, 100 + 38, BMP_Di, WHITE, BLACK); //        
        LCD_DrawImage1BPP(82 + 42, 100 + 38, BMP_Fengshan, WHITE, BLACK); //
        
        LCD_DrawImage1BPP(82, 100 + 38 * 2, BMP_Chesu, WHITE, BLACK); //高速风扇
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 4, BMP_Kongtiao, WHITE, BLACK); //空调
        
        LCD_DrawImage1BPP(82, 100 + 38 * 5, BMP_IC_Qian, WHITE, BLACK); //前模块PTC
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 6, BMP_IC_Hou, WHITE, BLACK); //后模块PTC
        ...
        for (i = 0; i < 3; i++) {
            LCD_DrawImage1BPP(82, 100 + 38 * (7 + i), BMP_DJ, WHITE, BLACK); //电机故障保护状态1
        ...
        for (i = 0; i < 2; i++) {
            LCD_DrawImage1BPP(82 + 320, 100 + 38 * i, BMP_DC, WHITE, BLACK); //电池故障状态1
        ...
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 2, BMP_YWBJQ, WHITE, BLACK);//烟雾报警器状态
        LCD_DrawImage1BPP(82 + 320 + 42 + 63, 100 + 38 * 2, BMP_Zhuangtai, WHITE, BLACK);
        
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 3 , BMP_YWBJQ, WHITE, BLACK); //烟雾报警
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 0, BMP_Zhengche, WHITE, BLACK); //整车故障等级
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 1, BMP_DJ, WHITE, BLACK); //电机系统故障等级
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 2, BMP_DC, WHITE, BLACK); //电池系统故障等级
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 3, BMP_Zhengche, WHITE, BLACK); //整车控制器故障等级
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 4, BMP_DCDC, WHITE, BLACK); //DCDC故障等级
        LCD_DrawImage1BPP(82 + 55, 100 + 38 * 4, BMP_Guzhang, WHITE, BLACK);
        LCD_DrawImage1BPP(82 + 55 + 42, 100 + 38 * 4, BMP_Dengji, WHITE, BLACK);

        LCD_DrawImage1BPP(82, 100 + 38 * 5, BMP_Chongdianqiang, WHITE, BLACK); //充电机故障等级
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 6, BMP_ZLZX, WHITE, BLACK); //助力转向故障等级
        LCD_DrawImage1BPP(82 + 84, 100 + 38 * 6, BMP_Guzhang, WHITE, BLACK);
        LCD_DrawImage1BPP(82 + 84 + 42, 100 + 38 * 6, BMP_Dengji, WHITE, BLACK);
        
        LCD_DrawImage1BPP(82, 100 + 38 * 7, BMP_Kongtiao, WHITE, BLACK); //空调故障等级
        ...
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 0, BMP_Jiasu, WHITE, BLACK); //加速
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 0, BMP_Taban, WHITE, BLACK); //踏板
        LCD_DrawImage1BPP(82 + 320 + 42 + 42, 100 + 38 * 0, BMP_Xingneng, WHITE, BLACK); //性能
        LCD_DrawImage1BPP(82 + 320 + 42 + 42 + 42, 100 + 38 * 0, BMP_Guzhang, WHITE, BLACK); //故障
        
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 1, BMP_Jiasu, WHITE, BLACK); //加速
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 1, BMP_Taban, WHITE, BLACK); //踏板
        LCD_DrawImage1BPP(82 + 320 + 42 + 42, 100 + 38 * 1, BMP_Dianyuan, WHITE, BLACK); //电源
        LCD_DrawImage1BPP(82 + 320 + 42 + 42 + 42, 100 + 38 * 1, BMP_Duanlu, WHITE, BLACK); //短路
                     
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 2, BMP_Jiasu, WHITE, BLACK); //加速
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 2, BMP_Taban, WHITE, BLACK); //踏板
        LCD_DrawImage1BPP(82 + 320 + 42 + 42, 100 + 38 * 2, BMP_Duidi, WHITE, BLACK); //对地
        LCD_DrawImage1BPP(82 + 320 + 42 + 42 + 42, 100 + 38 * 2, BMP_Duanlu, WHITE, BLACK); //短路
        
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 3, BMP_Jiasu, WHITE, BLACK); //加速
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 3, BMP_Taban, WHITE, BLACK); //踏板
        LCD_DrawImage1BPP(82 + 320 + 42 + 42, 100 + 38 * 3, BMP_Duanlu, WHITE, BLACK);//开路
        LCD_DrawImage1BPP(82 + 320 + 42 + 42, 100 + 38 * 3, BMP_Kai, WHITE, BLACK); 
        
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 4, BMP_Zhidong, WHITE, BLACK); //制动
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 4, BMP_Taban, WHITE, BLACK); //踏板
        LCD_DrawImage1BPP(82 + 320 + 42 + 42, 100 + 38 * 4, BMP_Xingneng, WHITE, BLACK); //性能
        LCD_DrawImage1BPP(82 + 320 + 42 + 42 + 42, 100 + 38 * 4, BMP_Guzhang, WHITE, BLACK); //故障
        
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 5, BMP_Zhidong, WHITE, BLACK); //制动
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 5, BMP_Taban, WHITE, BLACK); //踏板
        LCD_DrawImage1BPP(82 + 320 + 42 + 42, 100 + 38 * 5, BMP_Dianyuan, WHITE, BLACK); //电源
        LCD_DrawImage1BPP(82 + 320 + 42 + 42 + 42, 100 + 38 * 5, BMP_Duanlu, WHITE, BLACK); //短路
                     
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 6, BMP_Zhidong, WHITE, BLACK); //制动
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 6, BMP_Taban, WHITE, BLACK); //踏板
        LCD_DrawImage1BPP(82 + 320 + 42 + 42, 100 + 38 * 6, BMP_Duidi, WHITE, BLACK); //对地
        LCD_DrawImage1BPP(82 + 320 + 42 + 42 + 42, 100 + 38 * 6, BMP_Duanlu, WHITE, BLACK); //短路
        
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 7, BMP_Zhidong, WHITE, BLACK); //制动
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 7, BMP_Taban, WHITE, BLACK); //踏板
        LCD_DrawImage1BPP(82 + 320 + 42 + 42, 100 + 38 * 7, BMP_Duanlu, WHITE, BLACK);//开路
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 0, BMP_DC, WHITE, BLACK); //电池单体电压过低
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 1, BMP_DJ, WHITE, BLACK); //电机控制器温度过高
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 2, BMP_Xu, WHITE, BLACK); //蓄电池欠压
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 6, BMP_s, WHITE, BLACK); //SOC过低
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 7, BMP_Qibeng, WHITE, BLACK); //气泵故障等级
        LCD_DrawImage1BPP(82 + 42, 100 + 38 * 7, BMP_Guzhang, WHITE, BLACK); //
        LCD_DrawImage1BPP(82 + 42 + 42, 100 + 38 * 7, BMP_Dengji, WHITE, BLACK);
        
        LCD_DrawImage1BPP(82, 100 + 38 * 8, BMP_Shuibeng, WHITE, BLACK); //水泵故障
        LCD_DrawImage1BPP(82 + 42, 100 + 38 * 8, BMP_Guzhang, WHITE, BLACK); //

        LCD_DrawImage1BPP(82, 100 + 38 * 9, BMP_Xu, WHITE, BLACK); //蓄电池电压过低
        ...
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 6, BMP_Qibeng, WHITE, BLACK); //气泵压力过低
        ...
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 7, BMP_t, WHITE, BLACK); //TBOX离线
        ...
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 8, BMP_t, WHITE, BLACK); //TBOX SM失效
...
void MOTOUpdate(unsigned char index) {
    ...
    print_SZ_1(82 + 200 + 20, 100 - 8 + 38 * 5, Program, 3, 0, WHITE); //程序版本
    print_SZ_1(82 + 200 + 20, 100 - 8 + 38 * 6, VCU_LIFE, 3, 0, WHITE); //整车LIFE
    /*print_SZ_1(82 + 200 + 20 + 16, 100 - 8 + 38 * 7, Moto_Fault, 2, 0, WHITE); //电机故障代码
    print_SZ_1(82 + 200 + 20 + 16, 100 - 8 + 38 * 8, BAT_Fault, 2, 0, WHITE); //电池故障代码
    print_SZ_1(82 + 200 + 20 + 16, 100 - 8 + 38 * 9, VCU_Fault, 2, 0, WHITE); //整车故障代码*/

    print_SZ_1(82 + 200 + 320, 100 - 8, Speed_Percent * 4, 4, 1, WHITE); //加速踏板
    print_SZ_1(82 + 200 + 320, 100 - 8 + 38, Brake_Percent * 4, 4, 1, WHITE); //制动踏板
    updata_DAT(82 + 200 + 320, 100 + 38 * 2 - 8, Moto_Torque, 0.1, 3000, WHITE, 4, 0); //转矩
    print_SZ_1(82 + 200 + 320, 100 - 8 + 38 * 3, Vacuum, 3, 2, WHITE); //真空度
    print_SZ_1(82 + 200 + 320, 100 - 8 + 38 * 4, VCU_Speed, 3, 0, WHITE); //车速
    ...
    print_SZ_1(82 + 200 + 320+32, 100 - 8 + 38 * 9, BUS_Status, 1, 0, WHITE); //整车模式
    }
    else if(index == 2){
        if(WATER_PUMP){
            LCD_DrawImage1BPP(82 + 200, 100 + 38 * 0, BMP_Kaiqi, RED, BLACK); //开启
        }else{
            LCD_DrawImage1BPP(82 + 200, 100 + 38 * 0, BMP_Guanbi, WHITE, BLACK); //关闭
        }
        if(FAN_LOW){
            LCD_DrawImage1BPP(82 + 200, 100 + 38 * 1, BMP_Kaiqi, RED, BLACK); //开启
        }else{
            LCD_DrawImage1BPP(82 + 200, 100 + 38 * 1, BMP_Guanbi, WHITE, BLACK); //关闭
        }
        if(FAN_HIGH){
            LCD_DrawImage1BPP(82 + 200, 100 + 38 * 2, BMP_Kaiqi, RED, BLACK); //开启
        }else{
            LCD_DrawImage1BPP(82 + 200, 100 + 38 * 2, BMP_Guanbi, WHITE, BLACK); //关闭
        }
        if(AIR_PUMP){
            LCD_DrawImage1BPP(82 + 200, 100 + 38 * 3, BMP_Kaiqi, RED, BLACK); //开启
        }else{
            LCD_DrawImage1BPP(82 + 200, 100 + 38 * 3, BMP_Guanbi, WHITE, BLACK); //关闭
        }
        if(AC){
            LCD_DrawImage1BPP(82 + 200, 100 + 38 * 4, BMP_Kaiqi, RED, BLACK); //开启
        }else{
            LCD_DrawImage1BPP(82 + 200, 100 + 38 * 4, BMP_Guanbi, WHITE, BLACK); //关闭
        }
        ...
        for (i = 0; i < 8; i++) {
            if ((dump & 0x01) == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * i, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * i, BMP_Zhengchang, GREEN, BLACK); //正常
            }
            dump = dump >> 1;
        }
    } else if (index == 4) {
        ...
        for (i = 0; i < 7; i++) {
            ...
            if ((dump & 0x01) == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36, 100 + 38 * i, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36, 100 + 38 * i, BMP_Zhengchang, GREEN, BLACK); //正常
            }
            dump = dump >> 1;
        }
        ...
        if (VCU_WARN[2].bit3 == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36, 100 + 38 * 8, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36, 100 + 38 * 8, BMP_Zhengchang, GREEN, BLACK); //正常
            }

        if (VCU_WARN[2].bit4 == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36, 100 + 38 * 9, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36, 100 + 38 * 9, BMP_Zhengchang, GREEN, BLACK); //正常
            }
        
        if (VCU_WARN[2].bit5 == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 0, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 0, BMP_Zhengchang, GREEN, BLACK); //正常
            }
        
        if (VCU_WARN[2].bit6 == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 1, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 1, BMP_Zhengchang, GREEN, BLACK); //正常
            }
        
        if (VCU_WARN[2].bit7 == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 2, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 2, BMP_Zhengchang, GREEN, BLACK); //正常
            }
        
        if (VCU_WARN[2].bit8 == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 3, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 3, BMP_Zhengchang, GREEN, BLACK); //正常
            }
        
        if (VCU_WARN[3].bit1 == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 4, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 4, BMP_Zhengchang, GREEN, BLACK); //正常
            }
        
        if (VCU_WARN[3].bit2 == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 5, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 5, BMP_Zhengchang, GREEN, BLACK); //正常
            }
        
        if (VCU_WARN[3].bit3 == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 6, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 6, BMP_Zhengchang, GREEN, BLACK); //正常
            }
        
        if (VCU_WARN[3].bit4 == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 7, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * 7, BMP_Zhengchang, GREEN, BLACK); //正常
            }
        
        if (VCU_WARN[3].bit5 == 0x01) {
                LCD_DrawImage1BPP(82 + 200  + 36+ 320, 100 + 38 * 8, BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200  + 36+ 320, 100 + 38 * 8, BMP_Zhengchang, GREEN, BLACK); //正常
            }
    }
...
void BMSFrame(unsigned char index) {
    ...
    LCD_DrawImage1BPP(82 + 42, 100 + 38 * 4, BMP_Rongliang, WHITE, BLACK); //容量
    LCD_DrawImage1BPP(400 - 30, 100 + 38*4, BMP_a, WHITE, BLACK); //AH
    LCD_DrawImage1BPP(400 - 30+11, 100 + 38*4, BMP_h, WHITE, BLACK); //

    LCD_DrawImage1BPP(82, 100 + 38 * 5, BMP_Chongdianqiang, WHITE, BLACK); //充电机输出电压
    LCD_DrawImage1BPP(82 + 21, 100 + 38 * 5, BMP_DJ, WHITE, BLACK); //
    ...
    LCD_DrawImage1BPP(82, 100 + 38 * 6, BMP_Chongdianqiang, WHITE, BLACK); //充电机输出电流
    ...
    LCD_DrawImage1BPP(82, 100 + 38 * 8, BMP_Zhengche, WHITE, BLACK); //整车
    LCD_DrawImage1BPP(82 + 42, 100 + 38 * 8, BMP_Jueyuan, WHITE, BLACK); //绝缘
    LCD_DrawImage1BPP(82 + 42 + 42, 100 + 38 * 8, BMP_Guzhang, WHITE, BLACK); //故障

    LCD_DrawImage1BPP(82, 100 + 38 * 9, BMP_DC, WHITE, BLACK); //电池
    LCD_DrawImage1BPP(82 + 42, 100 + 38 * 9 ,BMP_Jueyuan, WHITE, BLACK); //绝缘
    LCD_DrawImage1BPP(82 + 42 + 42, 100 + 38 * 9, BMP_Guzhang, WHITE, BLACK); //故障

    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63, 100, BMP_Dan, WHITE, BLACK); //单
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21, 100, BMP_Ti, WHITE, BLACK); //体
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21, 100, BMP_Zui, WHITE, BLACK); //最
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21 + 21, 100, BMP_Gao, WHITE, BLACK); //高
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21 + 21 + 21, 100, BMP_DY, WHITE, BLACK); //电压
    LCD_DrawImage1BPP(400 + 320 - 50 - 10 + 63 - 24 - 63, 100, BMP_v, WHITE, BLACK); //V

    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 42 * 7, 100, BMP_Xiang_BMS, WHITE, BLACK); //箱

    LCD_DrawImage1BPP(82 + 320 - 55 + 63 + 42 * 8.5, 100, BMP_Hao, WHITE, BLACK); //号


    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63, 100 + 38, BMP_Dan, WHITE, BLACK); //单
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21, 100 + 38, BMP_Ti, WHITE, BLACK); //体
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21, 100 + 38, BMP_Zui, WHITE, BLACK); //最
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21 + 21, 100 + 38, BMP_Di, WHITE, BLACK); //低
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21 + 21 + 21, 100 + 38, BMP_DY, WHITE, BLACK); //电压
    LCD_DrawImage1BPP(400 + 320 - 50 - 10 + 63 - 24 - 63, 100 + 38, BMP_v, WHITE, BLACK); //V

    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 42 * 7, 100 + 38, BMP_Xiang_BMS, WHITE, BLACK); //箱

    LCD_DrawImage1BPP(82 + 320 - 55 + 63 + 42 * 8.5, 100 + 38, BMP_Hao, WHITE, BLACK); //号

    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63, 100 + 38 * 2, BMP_Dan, WHITE, BLACK); //单
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21, 100 + 38 * 2, BMP_Ti, WHITE, BLACK); //体
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21, 100 + 38 * 2, BMP_Zui, WHITE, BLACK); //最
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21 + 21, 100 + 38 * 2, BMP_Gao, WHITE, BLACK); //高
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21 + 21 + 21, 100 + 38 * 2, BMP_Wendu, WHITE, BLACK); //温度
    LCD_DrawImage1BPP(400 + 320 - 50 - 10 + 63 - 24 - 63, 100 + 38 * 2, BMP_T, WHITE, BLACK); //V

    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 42 * 7, 100 + 38 * 2, BMP_Xiang_BMS, WHITE, BLACK); //箱

    LCD_DrawImage1BPP(82 + 320 - 55 + 63 + 42 * 8.5, 100 + 38 * 2, BMP_Hao, WHITE, BLACK); //号

    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63, 100 + 38 * 3, BMP_Dan, WHITE, BLACK); //单
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21, 100 + 38 * 3, BMP_Ti, WHITE, BLACK); //体
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21, 100 + 38 * 3, BMP_Zui, WHITE, BLACK); //最
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21 + 21, 100 + 38 * 3, BMP_Di, WHITE, BLACK); //低
    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 21 + 21 + 21 + 21, 100 + 38 * 3, BMP_Wendu, WHITE, BLACK); //温度
    LCD_DrawImage1BPP(400 + 320 - 50 - 10 + 63 - 24 - 63, 100 + 38 * 3, BMP_T, WHITE, BLACK); //V

    LCD_DrawImage1BPP(82 + 320 - 50 - 10 + 63 + 42 * 7, 100 + 38 * 3, BMP_Xiang_BMS, WHITE, BLACK); //箱

    LCD_DrawImage1BPP(82 + 320 - 55 + 63 + 42 * 8.5, 100 + 38 * 3, BMP_Hao, WHITE, BLACK); //号
    
    for(i = 0;i < 6;i++){
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * (4 + i), BMP_Chongdianqiang, WHITE, BLACK); //充电机
        LCD_DrawImage1BPP(82 + 21 + 320, 100 + 38 * (4 + i), BMP_DJ, WHITE, BLACK); //
    }
    //LCD_DrawImage1BPP(82 + 320, 100 + 38 * 4, BMP_Chongdianqiang, WHITE, BLACK); //充电机LIFE
    //LCD_DrawImage1BPP(82 + 21 + 320, 100 + 38 * 4, BMP_DJ, WHITE, BLACK); //
    LCD_DrawImage1BPP(82 + 63 + 320 , 100 + 38 * 4, BMP_Life, WHITE, BLACK);
    
    //LCD_DrawImage1BPP(82 + 320, 100 + 38 * 5, BMP_Chongdianqiang, WHITE, BLACK); //充电机硬件故障
    ...
    LCD_DrawImage1BPP(82 + 63 + 320 , 100 + 38 * 6, BMP_Wendu, WHITE, BLACK);//充电机温度状态
    LCD_DrawImage1BPP(82 + 63 + 42 + 320 , 100 + 38 * 6, BMP_Zhuangtai, WHITE, BLACK);
    
    LCD_DrawImage1BPP(82 + 63 + 320 , 100 + 38 * 7, BMP_Input, WHITE, BLACK);//充电机输入状态
    LCD_DrawImage1BPP(82 + 63 + 42 + 320 , 100 + 38 * 7, BMP_Zhuangtai, WHITE, BLACK);
    
    LCD_DrawImage1BPP(82 + 63 + 320 , 100 + 38 * 8, BMP_Qidong, WHITE, BLACK);//充电机启动状态
    LCD_DrawImage1BPP(82 + 63 + 42 + 320 , 100 + 38 * 8, BMP_Zhuangtai, WHITE, BLACK);
    
    LCD_DrawImage1BPP(82 + 63 + 320 , 100 + 38 * 9, BMP_Tongxun, WHITE, BLACK);//充电机通讯状态
    LCD_DrawImage1BPP(82 + 63 + 42 + 320 , 100 + 38 * 9, BMP_Zhuangtai, WHITE, BLACK);
        ...
        LCD_DrawImage1BPP(82, 100, BMP_DC, WHITE, BLACK); //电池
        LCD_DrawImage1BPP(82 + 42, 100, BMP_DY, WHITE, BLACK); //电压
        LCD_DrawImage1BPP(82 + 42 + 42, 100, BMP_Junheng, WHITE, BLACK); //均衡
        LCD_DrawImage1BPP(82 + 42 + 42 + 42, 100, BMP_Guzhang, WHITE, BLACK); //故障

        LCD_DrawImage1BPP(82, 100 + 38, BMP_Dan, WHITE, BLACK); //单体温度过高
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 2, BMP_Zong, WHITE, BLACK); //总电压过高
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 3, BMP_Qieduan, WHITE, BLACK); //切断总负
        LCD_DrawImage1BPP(82 + 42, 100 + 38 * 3, BMP_Zong, WHITE, BLACK); //
        LCD_DrawImage1BPP(82 + 42 + 21, 100 + 38 * 3, BMP_Fuhe, WHITE, BLACK); //
        
        LCD_DrawImage1BPP(82, 100 + 38 * 4, BMP_Zong, WHITE, BLACK); //总电流过高
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 5, BMP_Pingjun, WHITE, BLACK); //平均电压过高
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 6, BMP_Dan, WHITE, BLACK); //单体电压过高
        ...
        LCD_DrawImage1BPP(82, 100 + 38 * 7, BMP_Dan, WHITE, BLACK); //单体电压过低
        ...
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 0, BMP_Rongliang, WHITE, BLACK); //容量低报警
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 0, BMP_Di, WHITE, BLACK); //
        LCD_DrawImage1BPP(82 + 320 + 42 + 21, 100 + 38 * 0, BMP_Baojing, WHITE, BLACK); //
        
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 1, BMP_XWJ, WHITE, BLACK); //下位机报警
        LCD_DrawImage1BPP(82 + 320 + 42 + 21, 100 + 38 * 1, BMP_Baojing, WHITE, BLACK); //
                    
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 2, BMP_Jueyuan, WHITE, BLACK); //绝缘
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 2, BMP_Guzhang, WHITE, BLACK); //故障

        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 3, BMP_DC, WHITE, BLACK); //电池
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 3 , BMP_Wendu, WHITE, BLACK); //温度
        LCD_DrawImage1BPP(82 + 320 + 42 + 42, 100 + 38*3, BMP_Junheng, WHITE, BLACK); //均衡
        LCD_DrawImage1BPP(82 + 320 + 42 + 42 + 42, 100 + 38*3, BMP_Guzhang, WHITE, BLACK); //故障
        
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 4, BMP_s, WHITE, BLACK); //SOC跳变故障
        ...
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 5, BMP_Pipei, WHITE, BLACK); //匹配
        LCD_DrawImage1BPP(82 + 320 + 42, 100 + 38 * 5, BMP_Guzhang, WHITE, BLACK); //故障
        
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 6, BMP_Guo, WHITE, BLACK); //过充报警故障
        ...
        LCD_DrawImage1BPP(82 + 320, 100 + 38 * 7, BMP_s, WHITE, BLACK); //SOC高故障
...
void BMSUpdate(unsigned char index) {   
    ...
    for (i = 0; i < 5; i++) {
            if ((dump & 0x01) == 0x01) {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * (i + 5), BMP_Guzhang, RED, BLACK); //故障
            } else {
                LCD_DrawImage1BPP(82 + 200 + 36 + 320, 100 + 38 * (i + 5), BMP_Zhengchang, GREEN, BLACK); //正常
            }
            dump = dump >> 1;
        }
    ...
    if (index == 1) {
        for (j = 0; j < 5; j++) {
            for (i = 0; i < 10; i++) {
                ...
                else print_SZ_1(65 + j * 160, 46 + 70 + 36 * i, BAT_Cell_Voltage[j * 10 + i + 1]*2, 3, 2, GREEN); //单体电压
            }
        }
    }
    ...
    if (index == 2) {
        for (j = 0; j < 5; j++) {
            for (i = 0; i < 10; i++) {
                ...
                else print_SZ_1(65 + j * 160, 46 + 70 + 36 * i, BAT_Cell_Voltage[50 + j * 10 + i + 1]*2, 3, 2, GREEN); //单体电压
            }
        }
    }
    ...
    if (index == 3) {
        for (j = 0; j < 5; j++) {
            for (i = 0; i < 10; i++) {
                ...
                else print_SZ_1(65 + j * 160, 46 + 70 + 36 * i, BAT_Cell_Voltage[100 + j * 10 + i + 1]*2, 3, 2, GREEN); //单体电压
            }
        }
    }
    ...
    if (index == 4) {
        for (j = 0; j < 5; j++) {
            for (i = 0; i < 10; i++) {
                ...
                else print_SZ_1(65 + j * 160, 46 + 70 + 36 * i, BAT_Cell_Voltage[150 + j * 10 + i + 1]*2, 3, 2, GREEN); //单体电压
            }
        }
    }
        ...
        for (i = 0; i < 10; i++) {
            for (j = 1; j < 4; j++) {
                if (BAT_Cell_Temperature[i][j - 1] == 0)
                    updata_DAT(65 + 23 + 42 + j * 160 - 10, 46 + 70 + 36 * i, BAT_Cell_Temperature[i][j - 1], 1, 40, BLACK, 3, 0); //单体温度
                else updata_DAT(65 + 23 + 42 + j * 160 - 10, 46 + 70 + 36 * i, BAT_Cell_Temperature[i][j - 1], 1, 40, GREEN, 3, 0); //单体温度
            }
        }
    }
    /*for (i = 0; i < 4; i++) {
