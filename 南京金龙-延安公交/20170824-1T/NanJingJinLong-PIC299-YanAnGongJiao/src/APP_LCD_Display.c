...
void disp_BMP_baojing(void) {
    ...
    if (VCU_Fault.bit1 || MCU_Fault.bit8) color = RED;
    else color = GRAY;
    ...
    if (Oum_Warn > 0 || Bcan_count == CAN_TIME) color = RED;
    else color = GRAY;
    ...
    if (AC_Status) color = GREEN;
    else if (AC_Fault_Code > 0) color = RED;
    else color = GRAY;
    ...
    if (VCU_KT_Status.bit3) color = YELLOW;
    else color = GRAY;
    ...
    if (VCU_Status.bit4 == 1) color = GREEN;
    else color = GRAY;
    ...
    if (alarm_update_flag == 0) { //3S 更新一次 
        switch (al_index) {
            ...
            case 37:
                al_index = 38;
                if (KT_FAULT) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_Jiechuqi, YELLOW, BLACK); //接?器故障
            ...
            case 38:
                al_index = 39;
                if (VCU_Status.bit3) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_System, YELLOW, BLACK); //系?互?故障
...
void frmMain(void) {
    if (f_index == 0) { //只更新一次
        ...
        f_index = 2;
        updata_DAT(700, 318 + 15 + 38, MCU_NM, 1, 2000, WHITE, 4, 0); //电机转矩
    } else if (f_index == 2) {
        f_index = 3;
        disp_BMP_baojing();
        print_SZ_1(305, 318 + 15 + 38, BAT_Max_Cell_Voltage[0], 4, 3, WHITE); //单体高电压
        print_SZ_1(305, 318 + 15 + 38 * 2, BAT_Min_Cell_Voltage[0], 4, 3, WHITE); //单体低电压
    } else if (f_index == 3) {
        f_index = 4;
        disp_miles();
        //门开关图像报警
        ...
        updata_DAT(540 + 50, 318 + 15 + 38 * 2, MCU_Control_Temp, 1, 40, WHITE, 3, 0); //电机控制器温度
        updata_DAT(540 + 50, 318 + 15 + 38, MCU_Temp, 1, 40, WHITE, 3, 0); //电机温度
    } else if (f_index == 5) {
        f_index = 6;
        updata_SOC(585, 155 - 10);
        updata_DAT(150 + 45, 318 + 15 + 38, BMS_V * 0.02, 1, 0, WHITE, 4, 0); //总电压
        //updata_DAT(150 + 45, 318 + 15 + 38 * 2, BMS_A * 0.1, 1, 3200, WHITE, 4, 0); //总电流
        updata_DCDL(150 + 45 + 16, 318 + 15 + 38 * 2); //总电流
    } else if (f_index == 6) {
        f_index = 7;
        updata_DAT(410 + 50, 318 + 15 + 38, BAT_Max_Cell_Temperature[0], 1, 40, WHITE, 3, 0); //单体高温
        updata_DAT(410 + 50, 318 + 15 + 38 * 2, BAT_Min_Cell_Temperature[0], 1, 40, WHITE, 3, 0); //单体低温       
...
void BatteryFrame(unsigned char index) {
    ...
    if (index == 1) {
        ...
        for (i = 0; i < 8; i++) {
            ...
            print_SZ_1(82 + 21, 100 - 8 + 38 * i, (index - 2) * 8 + i + 1, 2, 0, WHITE);
        ...
        for (i = 0; i < 8; i++) {
            ...
            print_SZ_1(82 + 320 + 21, 100 - 8 + 38 * i, (index - 2) * 8 + i + 1, 2, 0, WHITE);
            LCD_DrawImage1BPP(82 + 320 + 21 + 32, 100 + 38 * i, BMP_Fu, WHITE, BLACK); //负
...
void BatteryUpdate(unsigned char index) {
    ...
    if (index == 1) {
        ...
        for (i = 0; i < 8; i++) {
            updata_DAT(82 + 220, 100 - 8 + 38 * i, BAT_Pole_Temperature_P[(index - 2) * 8 + i], 1, 40, YELLOW, 3, 0);
        }

        for (i = 0; i < 8; i++) {
            updata_DAT(82 + 220 + 320, 100 - 8 + 38 * i, BAT_Pole_Temperature_N[(index - 2) * 8 + i], 1, 40, YELLOW, 3, 0);
        }
    } else if (index == 3) {
        for (i = 0; i < 8; i++) {
            updata_DAT(82 + 220, 100 - 8 + 38 * i, BAT_Pole_Temperature_P[(index - 2) * 8 + i], 1, 40, YELLOW, 3, 0);
