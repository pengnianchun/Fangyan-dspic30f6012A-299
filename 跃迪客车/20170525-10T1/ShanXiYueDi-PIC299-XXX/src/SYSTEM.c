...
void SYSTME_Logic(void) {
 ...
 // F_PO3  = (IN21 && M_ON);                    //喷水电机
    ...
    F_PO7  = (FLASH && (IN6 || wake_up1));      //左转向灯
    if((KEY_state&0x80)||fKL6)F_PO8  = 1;       //刹车灯
    else                      F_PO8  = 0;                         
    F_PO9  = (IN8 );                            //远光灯      
    F_PO10 = (IN11 && (F_PO9||F_PO13||F_PO4));  //后雾灯
    F_PO11 =  F_PO9;                            //远光灯
    F_PO12 = (IN13);                            //喇叭 
    F_PO13 = (IN7 && IN9 );                     //近光灯 
    F_PO14 = (IN7);                             //位置灯
...
void LED_Logic(void) {
    ...
    LED1  = fKL15;  //前门开指示
  //if(IN1) LED1=0;//取消前门关信号
    
    LED2  = fKH1;   //中门开指示   
  //if(IN2) LED2=0;//取消中门关信号
...
//蜂鸣器 为0时响，为1时不响
         if(    IN6               ||  //左转向灯报警 
           ...
           (   !fKL4 ||   //左前摩擦片报警
              ...
              //LED4               ||  //左转向灯报警 
              //LED8               ||  //右转向灯报警
               (IN12&&(Vspeed>100))||  //安全带未系
            
              ((YW_St[0][0]&0X0f)==0x04)||  //第1箱探测到报警
              ((YW_St[1][0]&0X0f)==0x04)||  //第2箱探测到报警   
              ((YW_St[2][0]&0X0f)==0x04)||  //第3箱探测到报警
              ((YW_St[3][0]&0X0f)==0x04)||  //第4箱探测到报警  
              ((YW_St[4][0]&0X0f)==0x04)||  //第5箱探测到报警 
...
void PCAN_send_yb(void) {
    ...
    temp.bit0 =(fKL10||fKL11); //气压低报警 (LED9||LED11)
    temp.bit1 = fKL8; //驻车信号 LED18
    temp.bit2 = 0;    //后仓门
    temp.bit3 = fKL15;//前门开   LED1
    temp.bit4 = fKH1; //后门开   LED2
    temp.bit5 = IN3;  //二挡  ON
    temp.bit6 = M_ON; //一档  ACC
    temp.bit7 = 0;
    msg.data[0] =temp.byte;
    
    temp.bit0 = F_PO10;//后雾灯  LED20
    temp.bit1 = F_PO13;//近光灯  LED22
    temp.bit2 = 0;     //发动机预热
    temp.bit3 = 0;     //油量报警
    temp.bit4 = F_PO6; //右转向灯LED8
    temp.bit5 = F_PO4; //前雾灯  LED19
    temp.bit6 = F_PO9; //远光灯  LED17
    temp.bit7 = F_PO7; //左转向灯LED4
...
if(xz==0){
    ...
    temp1=ADR[2];//电阻式
    if(temp1>10 && temp1<180)temp1=(unsigned int)(57.143*(temp1-10));  
    else                     temp1=0;
...
void PCAN_Send_TCO2(void) {
    ...
    temp.bit2 =(F_PO4||F_PO10);//雾灯
    temp.bit3 = F_PO5; //倒车灯
    temp.bit4 = F_PO12;//喇叭      
    msg.data[0] = temp.byte;            
            
    temp.bit1 = 0;    //严重故障
    temp.bit2 = F_PO6;//右转向灯
    temp.bit3 = F_PO7;//左转向灯
    temp.bit4 = (fKL15||fKH1);//车门开      
    msg.data[1] = temp.byte;   
    
    temp.bit1 = 0;//门强制选择
    temp.bit2 = 0;//
    temp.bit3 = ( ((ADV[3]/10)<200)||((ADV[4]/10)<200) );//低气压报警
    temp.bit4 = F_PO9;//远光灯      
    ...
    temp.bit1 = ((ADV[3]/10)<200);//前气压报警
    temp.bit2 = ((ADV[4]/10)<200);//后气压报警
    temp.bit3 = 0;//
    temp.bit4 = 0;//      
...
unsigned char  YW_St[5][5]={//烟雾报警
                         //{0x01,0x01,0x01,0x64,0x20},//1
                         //{0x02,0x02,0x02,0x54,0x30},//2
                         //{0x03,0x01,0x01,0x44,0x40},//3
                         //{0x04,0x02,0x02,0x34,0x50},//4                 
                         //{0x00,0x01,0x01,0x24,0x60},//5
                           
                           {0x01,0x00,0x00,0x28,0x00},//1
                           {0x01,0x00,0x00,0x28,0x00},//2
                           {0x01,0x00,0x00,0x28,0x00},//3
                           {0x01,0x00,0x00,0x28,0x00},//4                 
                           {0x01,0x00,0x00,0x28,0x00},//5            
                           };
    ...
    if (C2RX0CONbits.RXFUL) { //数据有效
        ...
        switch (msg.id) {
            ...
            case 0x18FFC09E: //传统空调
                ...
                KT_Status[1]=msg.data[1];  
                KT_Status[2]=msg.data[2];   
                KT_temp[0]  =msg.data[3];
                KT_temp[1]  =msg.data[4];
                KT_temp[2]  =msg.data[5];
                KT_ctrl     =msg.data[6];
                KT_life     =msg.data[7];
                break;
            ...
            default:
                ...
                if((msg.id>=0x18FF5A51) && (msg.id<=0x18FF5A55)){//烟雾报警器
                    i=((unsigned char)((msg.id & 0x0000000F))-0x01); 
                    YW_St[i][0] = msg.data[0]; //烟雾报警器状态
                    YW_St[i][1] = msg.data[1]; //烟雾及火焰报警状态      
                    YW_St[i][2] = msg.data[2]; //温度报警状态
                    YW_St[i][3] = msg.data[3]; //温度报警状态
                    YW_St[i][4] = msg.data[7]; //温度报警状态    
                } 
