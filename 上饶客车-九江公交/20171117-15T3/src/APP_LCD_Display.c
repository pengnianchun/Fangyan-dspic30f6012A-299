...
void disp_BMP_baojing(void) {
    ...
    if (EV_DTC.Insulation_Fault > 0) color = RED;
    else color = GRAY;
    ...
    if (Red_Stop_Lamp_Status == 1) color = RED;
    else if (Amber_Warning_Lamp_Status == 1) color = YELLOW;
    else color = GRAY;
    ...
    if (!EV_DTC.KT_SIGNAL) color = YELLOW;
    else color = GRAY;
    ...
    if (EV_DTC.MT_Temp_Warn) color = RED;
    else color = GRAY;
    ...
    if (EV_DTC.ev_ready_flag) color = GREEN;
    else color = GRAY;
    ...
    if (EV_DTC.Fire_Fault) color = YELLOW;
    else color = GRAY;
    if (color_old[18] != color) {
        color_old[18] = color;
        LCD_DrawImage1BPP(140 + 66 * 6, 74 - 5, Bmp_baojing[15], color, BLACK); //烟雾火灾报警
    }
...
void disp_FONT_baojing(unsigned int x, unsigned int y) {
    ...
    if (alarm_update_flag == 0) { //3S 更新一次 
        switch (al_index) {
            ...
            case 6:
                al_index = 7;
                if (EV_DTC.MCU_COMM_Fault) {
                    LCD_DrawImageSBPP(x, y + 8, 11, MCU, YELLOW, BLACK); //MCU通讯故障
                    LCD_DrawImage1BPP(x + 11 + 11 + 11, y + 8, BMP_Tongxin, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 11 + 11 + 11 + 42, y + 8, BMP_Guzhang, YELLOW, BLACK);
                    LCD_DrawRect(x + 11 + 11 + 11 + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
            ...
            case 16:
                al_index = 17;
                if (EV_DTC.SOC_Warn > 0) {
                    LCD_DrawImageSBPP(x, y + 8, 11, SOC, YELLOW, BLACK); //SOC报警
                    //LCD_DrawImage1BPP(x + 11 + 11 + 11, y + 8, BMP_Di, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 11 + 11 + 11, y + 8, BMP_Baojing, YELLOW, BLACK);
                    LCD_DrawRect(x + 11 + 11 + 11 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                }
            ...
            case 29:
                al_index = 30;
                if (EV_DTC.Fire_Fault) {
                    LCD_DrawImageSBPP(x, y + 8, 42, HZBJGZ, YELLOW, BLACK); //火灾报警故障
                    LCD_DrawRect(x + 42 + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                }
            ...
            case 41:
                al_index = 42;
                if (Ev_Fail_Mode > 0) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_v, YELLOW, BLACK); //VCU系统故障
            ...
            case 48:
                al_index = 49;
                if (EV_DTC.DCAC_A_Fault == 1) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_IC_Zhuanxiang, YELLOW, BLACK); //转向DCAC故障       
                    LCD_DrawImage1BPP(x + 42, y + 8 - 2, BMP_DCAC, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 52 + 42, y + 8, BMP_Guzhang, YELLOW, BLACK);
                    LCD_DrawRect(x + 52 + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                }
            ...
            case 49:
                al_index = 50;
                if (EV_DTC.DCAC_B_Fault == 1) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_Zhidong, YELLOW, BLACK); //制动DCAC故障       
                    LCD_DrawImage1BPP(x + 42, y + 8 - 2, BMP_DCAC, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 52 + 42, y + 8, BMP_Guzhang, YELLOW, BLACK);
                    LCD_DrawRect(x + 52 + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
            ...
            case 50:
                al_index = 51;
                if (EV_DTC.A_Fault == 1) {
                    LCD_DrawImageSBPP(x, y + 8, 42, YMTBGZ, YELLOW, BLACK); //油门踏板故障       
                    LCD_DrawRect(x + 65 + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                }
            ...
            case 51:
                al_index = 52;
                if (EV_DTC.B_Fault == 1) {
                    LCD_DrawImageSBPP(x, y + 8, 42, ZDTBGZ, YELLOW, BLACK); //制动踏板故障       
                    LCD_DrawRect(x + 65 + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                }
...
void updata_Dangwei(unsigned int x, unsigned int y) {
    ...
    gear = EV_Gear;
    if (gear == 3) gear = 0;
    else if (gear == 4) gear = 1;
    else if (gear == 2) gear = 2;
...
void frmMain(void) {
    if (f_index == 0) { //只更新一次
        ...
        f_index = 2;
        print_SZ_1(700, 318 + 15 + 36 - 5, EV_RestAhr, 3, 0, WHITE); //电池剩余电量
    } else if (f_index == 2) {
        f_index = 3;
        disp_BMP_baojing();
        print_SZ_1(305, 318 + 15 + 36 - 5, BAT_Max_Cell_Voltage, 3, 2, WHITE); //单体高电压
        print_SZ_1(305, 318 + 15 + 36 * 2 - 5, BAT_Min_Cell_Voltage, 3, 2, WHITE); //单体低电压
    } else if (f_index == 3) {
        f_index = 4;
        disp_miles(10, 318 - 5);
        //门开关图像报警
        ...
        updata_DAT(150 + 45, 318 + 15 + 36 - 5, BMS_V, 1, 0, WHITE, 4, 0); //总电压
...
void ICFrame(unsigned char index) { //仪表框架
    ...
    if (index == 1) {
        ...
        //IN2  制动开关
        LCD_DrawImage1BPP(35, 75 + 25 * 1, BMP_Zhidong, WHITE, BLACK);
        LCD_DrawImage1BPP(35 + 42, 75 + 25 * 1, BMP_IC_Kaiguan, WHITE, BLACK);
        //IN3  
        //IN4  START档
        ...
        //IN7  近光灯
        LCD_DrawImageSBPP(35, 75 + 25 * 6, 21, JGDKG, WHITE, BLACK);
        //IN8  远光灯
        LCD_DrawImageSBPP(35, 75 + 25 * 7, 21, YGDKG, WHITE, BLACK);
        //IN9  前雾灯
        ...
        //IN12 路牌开关
        LCD_DrawImageSBPP(35, 75 + 25 * 11, 42, LPKG, WHITE, BLACK);
        //IN13 ABS故障
        LCD_DrawImageSBPP(35, 75 + 25 * 12, 11, ABS, WHITE, BLACK);
        //IN14
        //IN15
        //IN16
        /*************************第二列开关***********************************/
        ...
        //IN19 
        //IN20 前门开开关
        LCD_DrawImageSBPP(35, 75 + 25 * 3, 21, QMKKG, WHITE, BLACK);
        //IN21
        //IN22 风扇开关
        LCD_DrawImageSBPP(285, 75 + 25 * 5, 42, FSKG, WHITE, BLACK);
        //IN23 广告开关1
        LCD_DrawImageSBPP(285, 75 + 25 * 6, 42, GGKG1, WHITE, BLACK);
        //IN24 广告开关2
        LCD_DrawImageSBPP(285, 75 + 25 * 7, 42, GGKG2, WHITE, BLACK);
        //IN25 ASR故障
        //LCD_DrawImageSBPP(285, 75 + 25 * 8, 11, ASR, WHITE, BLACK);
        //IN26 中门关开关
        ...
        //
        //wake_up1 危险报警开关
        LCD_DrawImageSBPP(535, 75 + 25 * 5, 42, WXBJHXKG, WHITE, BLACK);
        //wake_up2 ACC唤醒开关
        ...
        /*********************顶模块输出***************************************/
        //OUT1  位置灯
        LCD_DrawImage1BPP(60, 75 + 25 * 0, BMP_Weizhi, WHITE, BLACK);
        LCD_DrawImage1BPP(60 + 42, 75 + 25 * 0, BMP_IC_Deng, WHITE, BLACK);
        //OUT2  前门开输出
        LCD_DrawImageSBPP(60, 75 + 25 * 1, 21, QMKSC, WHITE, BLACK);
        //OUT3  前门关输出
        LCD_DrawImageSBPP(60, 75 + 25 * 2, 21, QMGSC, WHITE, BLACK);
        //OUT4  中门开输出
        LCD_DrawImageSBPP(60, 75 + 25 * 3, 21, ZMKSC, WHITE, BLACK);
        //OUT5  中门关输出
        LCD_DrawImageSBPP(60, 75 + 25 * 4, 21, ZMGSC, WHITE, BLACK);
        //OUT6  左转向灯
        ...
        LCD_DrawImage1BPP(60 + 63, 75 + 25 * 6, BMP_IC_Deng, WHITE, BLACK);
        //OUT8 前雾灯输出
        LCD_DrawImageSBPP(60, 75 + 25 * 7, 21, QWDSC, WHITE, BLACK);
        //OUT9  喇叭输出
        LCD_DrawImageSBPP(60, 75 + 25 * 8, 42, LBSC, WHITE, BLACK);
        //OUT10 前路牌输出
        LCD_DrawImageSBPP(60, 75 + 25 * 9, 21, QLP, WHITE, BLACK);
        //OUT11 后路牌输出
        LCD_DrawImageSBPP(60, 75 + 25 * 10, 21, HLP, WHITE, BLACK);
        //OUT12 左远光灯
        LCD_DrawImageSBPP(60, 75 + 25 * 11, 21, ZYGDSC, WHITE, BLACK);
        //OUT13 右远光灯
        LCD_DrawImageSBPP(60, 75 + 25 * 12, 21, YYGDSC, WHITE, BLACK);
        //OUT14 左近光灯输出
        LCD_DrawImageSBPP(60, 75 + 25 * 13, 21, ZJGDSC, WHITE, BLACK);
        //OUT15 右近光灯输出
        LCD_DrawImageSBPP(60, 75 + 25 * 14, 21, YJGDSC, WHITE, BLACK);
        /************************顶模块开关*********************************/
        //KL1  
        ...
        //KL11 
        //KL12 中门开信号
        LCD_DrawImageSBPP(285 + 150, 75 + 25 * 11, 21, ZMKXH, WHITE, BLACK);
        //KL13 地址选择1
        LCD_DrawImage1BPP(285 + 150, 75 + 25 * 12, BMP_Dizhi, WHITE, BLACK);
        LCD_DrawImage1BPP(285 + 150 + 42, 75 + 25 * 12, BMP_Xuanze, WHITE, BLACK);
        print_SZ_2(285 + 150 + 84 + 6, 75 + 25 * 12, 1, 1, 0, WHITE);
        ...
        print_SZ_2(285 + 150 + 84 + 6, 75 + 25 * 13, 2, 1, 0, WHITE);
        //KL15 应急阀报警
        LCD_DrawImage1BPP(285 + 150, 75 + 25 * 14, BMP_Yingjifa, WHITE, BLACK);
        LCD_DrawImage1BPP(285 + 150 + 63, 75 + 25 * 14, BMP_Baojing, WHITE, BLACK);
        //KH1  前门开信号
...
void MOTOFrame(unsigned char index) {
    ...
    if (index == 1) {
        LCD_DrawImageSBPP(82, 100 + 38 * 0, 42, DJZS, WHITE, BLACK); //电机转速
        LCD_DrawImage1BPP(400 - 24 - 24, 100 + 38 * 0, BMP_RPM, WHITE, BLACK); //RPM

        LCD_DrawImageSBPP(82, 100 + 38 * 1, 42, DJSRDY, WHITE, BLACK); //电机输入电压
        LCD_DrawImage1BPP(400 - 24, 100 + 38 * 1, BMP_v, WHITE, BLACK); //V

        LCD_DrawImageSBPP(82, 100 + 38 * 2, 42, DJWD, WHITE, BLACK); //电机温度
        LCD_DrawImage1BPP(400 - 28, 100 + 38 * 2, BMP_T, WHITE, BLACK); //℃

        LCD_DrawImage1BPP(82, 100 + 38 * 3, BMP_KZQ, WHITE, BLACK); //控制器
        LCD_DrawImage1BPP(82 + 63, 100 + 38 * 3, BMP_Wendu, WHITE, BLACK); //温度
        LCD_DrawImage1BPP(400 - 28, 100 + 38 * 3, BMP_T, WHITE, BLACK); //℃

        LCD_DrawImageSBPP(82, 100 + 38 * 4, 21, DW, WHITE, BLACK); //档位

        LCD_DrawImage1BPP(82, 100 + 38 * 5, BMP_Chesu, WHITE, BLACK); //车速
        LCD_DrawImage1BPP(400 - 48, 100 + 38 * 5, Bmp_kmh, WHITE, BLACK);
        ...
        LCD_DrawImageSBPP(82 + 320, 100 + 38 * 9, 42, ZDTBGZ, WHITE, BLACK); //制动踏板故障       
        /*
...
void MOTOUpdate(unsigned char index) {
    if (index == 1) {
        print_SZ_1(82 + 200, 100 - 8 + 38 * 0, MCU_RPM * 50, 4, 0, WHITE);
        print_SZ_1(82 + 200, 100 - 8 + 38 * 1, MCU_Voltage * 4, 4, 0, WHITE);
        updata_DAT(82 + 200 + 16, 100 - 8 + 38 * 2, MCU_Temp, 1, 40, WHITE, 3, 0);
        updata_DAT(82 + 200 + 16, 100 - 8 + 38 * 3, MCU_Control_Temp, 1, 40, WHITE, 3, 0);
        if (EV_Gear == 1) {
            LCD_DrawImage1BPP(82 + 42 * 6 - 5, 100 + 38 * 4, BMP_p, GREEN, BLACK);
        } else if (EV_Gear == 2) {
            LCD_DrawImage1BPP(82 + 42 * 6 - 5, 100 + 38 * 4, BMP_r, GREEN, BLACK);
        } else if (EV_Gear == 3) {
            LCD_DrawImage1BPP(82 + 42 * 6 - 5, 100 + 38 * 4, BMP_n, GREEN, BLACK);
        } else if (EV_Gear == 4) {
            LCD_DrawImage1BPP(82 + 42 * 6 - 5, 100 + 38 * 4, BMP_d, GREEN, BLACK);
        }
        print_SZ_1(82 + 200 + 20, 100 - 8 + 38 * 5, EV_Speed, 3, 0, WHITE);
        print_SZ_1(82 + 200 + 20 + 16, 100 - 8 + 38 * 6, EV_Voltage, 2, 0, WHITE); //蓄电池电压
        ...
        if (EV_DTC.MT_Fault == 1) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 0, BMP_Guzhang, RED, BLACK);
        } else if (EV_DTC.MT_Fault == 0) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 0, BMP_Zhengchang, GREEN, BLACK);
        }
        if (EV_DTC.MCU_COMM_Fault == 1) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 1, BMP_Guzhang, RED, BLACK);
        } else if (EV_DTC.MCU_COMM_Fault == 0) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 1, BMP_Zhengchang, GREEN, BLACK);
        }

        if (EV_DTC.BAT_Fault == 1) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 2, BMP_Guzhang, RED, BLACK);
        } else if (EV_DTC.BAT_Fault == 0) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 2, BMP_Zhengchang, GREEN, BLACK);
        }

        if (EV_DTC.BMS_COMM_Fault == 1) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 3, BMP_Guzhang, RED, BLACK);
        } else if (EV_DTC.BMS_COMM_Fault == 0) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 3, BMP_Zhengchang, GREEN, BLACK);
        }

        if (EV_DTC.BOX_Fault == 1) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 4, BMP_Guzhang, RED, BLACK);
        } else if (EV_DTC.BOX_Fault == 0) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 4, BMP_Zhengchang, GREEN, BLACK);
        }

        if (EV_DTC.DCDC_Fault == 1) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 5, BMP_Guzhang, RED, BLACK);
        } else if (EV_DTC.DCDC_Fault == 0) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 5, BMP_Zhengchang, GREEN, BLACK);
        }

        if (EV_DTC.DCAC_A_Fault == 1) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 6, BMP_Guzhang, RED, BLACK);
        } else if (EV_DTC.DCAC_A_Fault == 0) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 6, BMP_Zhengchang, GREEN, BLACK);
        }

        if (EV_DTC.DCAC_B_Fault == 1) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 7, BMP_Guzhang, RED, BLACK);
        } else if (EV_DTC.DCAC_B_Fault == 0) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 7, BMP_Zhengchang, GREEN, BLACK);
        }

        if (EV_DTC.A_Fault == 1) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 8, BMP_Guzhang, RED, BLACK);
        } else if (EV_DTC.A_Fault == 0) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 8, BMP_Zhengchang, GREEN, BLACK);
        }

        if (EV_DTC.B_Fault == 1) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 9, BMP_Guzhang, RED, BLACK);
        } else if (EV_DTC.B_Fault == 0) {
            LCD_DrawImage1BPP(82 + 320 + 42 * 6 - 5, 100 + 38 * 9, BMP_Zhengchang, GREEN, BLACK);
        }
...
void BMSFrame(unsigned char index) {
    ...
    if (index == 1) {
        ...
        LCD_DrawImageSBPP(82, 100 + 38 * 5, 42, DCSYDL, WHITE, BLACK); //电池剩余电量
        LCD_DrawImage1BPP(400 - 24 - 11 - 11, 100 + 38 * 5, BMP_k, WHITE, BLACK); //KWH
        LCD_DrawImage1BPP(400 - 24 - 11, 100 + 38 * 5, BMP_w, WHITE, BLACK);
...
void BMSUpdate(unsigned char index) {
    if (index == 1) {
        ...
        updata_DAT(82 + 200 + 30, 100 - 8 + 38, BMS_A, 4, 500, WHITE, 3, 0);
        print_SZ_1(82 + 200 + 30, 100 - 8 + 38 * 2, BMS_SOC, 3, 0, WHITE);
        print_SZ_1(82 + 200 + 30 - 16 * 5, 100 - 8 + 38 * 3, EV_Kwh_avg, 3, 0, WHITE);
        print_SZ_1(82 + 200 + 30 - 16 * 5, 100 - 8 + 38 * 4, EV_Kwh_1s, 3, 0, WHITE);
        print_SZ_1(82 + 200 + 30 - 16, 100 - 8 + 38 * 5, EV_RestAhr, 3, 0, WHITE);

        print_SZ_1(82 + 200 + 300 - 32 - 5, 100 - 8, BAT_Max_Cell_Voltage, 3, 2, WHITE);
        //print_SZ_1(82 + 200 + 300 - 32 + 16 * 6 - 5, 100 - 8, BAT_Max_Cell_Voltage_Cell_Case, 3, 0, WHITE);
        print_SZ_1(82 + 200 + 300 - 32 + 16 * 9 + 5, 100 - 8, BAT_Max_Cell_Voltage_Cell_Number, 4, 0, WHITE);

        print_SZ_1(82 + 200 + 300 - 32 - 5, 100 - 8 + 38, BAT_Min_Cell_Voltage, 3, 2, WHITE);
        //print_SZ_1(82 + 200 + 300 - 32 + 16 * 6 - 5, 100 - 8 + 38, BAT_Min_Cell_Voltage_Cell_Case, 3, 0, WHITE);
        print_SZ_1(82 + 200 + 300 - 32 + 16 * 9 + 5, 100 - 8 + 38, BAT_Min_Cell_Voltage_Cell_Number, 4, 0, WHITE);

        updata_DAT(82 + 200 + 300 - 5, 100 - 8 + 38 * 2, BAT_Max_Cell_Temperature, 1, 40, WHITE, 3, 0);
        print_SZ_1(82 + 200 + 300 - 32 + 16 * 9 + 5, 100 - 8 + 38 * 2, BAT_Max_Cell_Temp_Cell_Number, 4, 0, WHITE);

        updata_DAT(82 + 200 + 300 - 5, 100 - 8 + 38 * 3, BAT_Min_Cell_Temperature, 1, 40, WHITE, 3, 0);
        print_SZ_1(82 + 200 + 300 - 32 + 16 * 9 + 5, 100 - 8 + 38 * 3, BAT_Min_Cell_Temp_Cell_Number, 4, 0, WHITE);
...
void update_setDATE(void) {
    ...
    temp = (unsigned int) (EV_Speed / 2 * new_pluse / 100);
    //车速
