...
unsigned char BMS_A = 125; //电池电流
unsigned char MCU_RPM;     //电机转速
unsigned char MCU_Voltage; //电机母线电压
unsigned char MCU_Temp = 40;    //电机温度
unsigned char MCU_Control_Temp = 40; //电机控制器温度
//EV Information Message1 0x18EF4BEF
unsigned char EV_Speed;
unsigned char EV_Kwh_avg;  //平均电耗
unsigned char EV_Kwh_1s;   //瞬时电耗
unsigned int  EV_RestAhr;  //电池剩余容量
unsigned char EV_Voltage;  //蓄电池电压
unsigned char EV_Gear;     //档位

//EV Information Message2 0x18EF4CEF
unsigned int  BAT_Max_Cell_Voltage; //电池单体高电压
unsigned int  BAT_Max_Cell_Voltage_Cell_Number; //最高单体电压编号
unsigned int  BAT_Min_Cell_Voltage; //电池单体低电压
unsigned int  BAT_Min_Cell_Voltage_Cell_Number; //最低单体电压编号

//EV Information Message3 0x18EF4DEF
unsigned char BAT_Max_Cell_Temperature = 40; //最高单体温度
unsigned int  BAT_Max_Cell_Temp_Cell_Number;   //最高单体温度编号
unsigned char BAT_Min_Cell_Temperature = 40; //最低单体温度
unsigned int  BAT_Min_Cell_Temp_Cell_Number;   //最低单体温度编号

//EV DTC Message 0x18EF4EEF
ID_18EF4EEF EV_DTC;

//VCU_DM1 0x18FECAEF
unsigned char Ev_Fail_Mode;
DM1 VCU_DM1;
unsigned char VCU_Occurrence_Count;

//Transmission Faults (DM1 with SA03)TCU_DTC 0x18FECA03
unsigned char Red_Stop_Lamp_Status;
unsigned char Amber_Warning_Lamp_Status;
DM1 TCU_DM1;
unsigned char TCU_Occurrence_Count;
...
unsigned char OILDCAC_Temp = 40;   //DCAC散热器温度
...
unsigned char AIRDCAC_Temp = 40;   //DCAC散热器温度
...
unsigned int  BAT_Pack_Voltage[25]; //电池箱电压
...
unsigned int  BMS_HighChargeCurrent;//最高充电电流
...
unsigned char BAT_Max_Pole_Temp; //最高极柱温度
...
unsigned char BAT_Pack_Number[25]; //电池箱串联数量
//电池报警信息 0x18B1EFF3
...
unsigned int  BAT_Cell_Voltage[10][35]; //电池单体电压
//电池组所有温度信息 0x18E1EFF3
unsigned int  BAT_Cell_TempNumber; //动力电池箱号
unsigned int  BAT_Cell_Temp[10][30]; //电池单体温度
//整车绝缘监控报文 0x1819A1A4
...
DATA_BIT Protect[2];
//集中润滑
DATA_BIT Concentrated_Lubrication;
*/
...
void __attribute__((__interrupt__, auto_psv)) _C2Interrupt(void) {
    ...
    if (C2RX0CONbits.RXFUL) { //数据有效
        ...
        switch (msg.id) {  
            case 0x18EF4AEF:
                Vcan_count = 0;
                BMS_V = ((unsigned char) msg.data[0] + ((unsigned int) msg.data[1] << 8));
                BMS_SOC = msg.data[2];
                BMS_A = msg.data[3];
                MCU_RPM = msg.data[4];
                MCU_Voltage = msg.data[5];
                MCU_Control_Temp = msg.data[6];
                MCU_Temp = msg.data[7];               
                break;
            ...
            case 0x18EF4BEF:
                Vcan_count = 0;
                EV_Speed = msg.data[0];
                EV_Kwh_avg = msg.data[1];
                EV_Kwh_1s = msg.data[2];
                EV_RestAhr = ((unsigned char) msg.data[4] + ((unsigned int) msg.data[5] << 8));
                EV_Voltage = msg.data[6] & 0x1F;
                EV_Gear = (msg.data[6] >> 5) & 0x07;
                break;
                
            case 0x18EF4CEF:
                Vcan_count = 0;
                BAT_Max_Cell_Voltage = ((unsigned char) msg.data[0] + ((unsigned int) msg.data[1] << 8));
                BAT_Max_Cell_Voltage_Cell_Number = ((unsigned char) msg.data[2] + ((unsigned int) msg.data[3] << 8));
                BAT_Min_Cell_Voltage = ((unsigned char) msg.data[4] + ((unsigned int) msg.data[5] << 8));
                BAT_Min_Cell_Voltage_Cell_Number = ((unsigned char) msg.data[6] + ((unsigned int) msg.data[7] << 8));
                break;

            case 0x18EF4DEF:
                Vcan_count = 0;
                BAT_Max_Cell_Temperature = (unsigned char) msg.data[0];
                BAT_Max_Cell_Temp_Cell_Number = ((unsigned char) msg.data[1] + ((unsigned int) msg.data[2] << 8));
                BAT_Min_Cell_Temperature = (unsigned char) msg.data[3];
                BAT_Min_Cell_Temp_Cell_Number = ((unsigned char) msg.data[4] + ((unsigned int) msg.data[5] << 8));
                break;

            case 0x18EF4EEF:
                Vcan_count = 0;
                for (i = 0; i < 5; i++) {
                    EV_DTC.byte[i] = msg.data[i];
                }
                break;

            case 0x18FECAEF:
                Vcan_count = 0;
                Ev_Fail_Mode = msg.data[0] & 0x0F;
                VCU_DM1.DTC1.bData[0] = msg.data[2];
                VCU_DM1.DTC1.bData[1] = msg.data[3];
                VCU_DM1.DTC1.bData[2] = msg.data[4];
                VCU_Occurrence_Count = msg.data[5];
                break;
                
            case 0x18FECA03:
                Vcan_count = 0;
                Red_Stop_Lamp_Status = msg.data[0] & 0x03;
                Amber_Warning_Lamp_Status = (msg.data[0] >> 2) & 0x03;
                TCU_DM1.DTC1.bData[0] = msg.data[2];
                TCU_DM1.DTC1.bData[1] = msg.data[3];
                TCU_DM1.DTC1.bData[2] = msg.data[4];
                TCU_Occurrence_Count = msg.data[5];
                break;
                
            case 0x18FFAFF3:
                Charge.byte = msg.data[0];
                break;
                
            case 0x18FF12F7:
                DCDC_Output_Voltage = ((unsigned char) msg.data[0] + ((unsigned int) msg.data[1] << 8));
                DCDC_Output_Current = ((unsigned char) msg.data[2] + ((unsigned int) msg.data[3] << 8));
                DCDC_WorkStatus = msg.data[4];
                DCDC_FaultCode = msg.data[5];
                break;    
                
            case 0x18019888:
                OILDCAC_Voltage = ((unsigned char) msg.data[2] + ((unsigned int) msg.data[3] << 8));
                OILDCAC_Current = ((unsigned char) msg.data[4] + ((unsigned int) msg.data[5] << 8));
                OILDCAC_Temp = msg.data[6];
                OILDCAC_FaultCode = msg.data[7];
                break;  
                
            case 0x1805A89C:
                AIRDCAC_InputVoltage = ((unsigned char) msg.data[0] + ((unsigned int) msg.data[1] << 8));
                AIRDCAC_Voltage = ((unsigned char) msg.data[2] + ((unsigned int) msg.data[3] << 8));
                AIRDCAC_Current = ((unsigned char) msg.data[4] + ((unsigned int) msg.data[5] << 8));
                AIRDCAC_Temp = msg.data[6];
                AIRDCAC_FaultCode = msg.data[7];
                break;    
            ...
            case 0x18A7EFF3:
                ...
                //BAT_Pack_SequenceNumber_N[1] = msg.data[2];
                BAT_Pack_Number[temp + 1] = msg.data[3];
                //BAT_Pack_SequenceNumber_N[2] = msg.data[4];
                BAT_Pack_Number[temp + 2] = msg.data[5];
                //BAT_Pack_SequenceNumber_N[3] = msg.data[6];
