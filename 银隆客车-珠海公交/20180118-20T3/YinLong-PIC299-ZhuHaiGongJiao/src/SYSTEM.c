...
void LED_Logic(void) {
    static unsigned char buzz_flag[2] = {1,1}, timer[2], old_LED2;
    LED1 = mKH1; //前门开指示
    ...
    LED6 = (((BMS_Status_Flag5 & 0x01) == 1) && LED7); //充电状态

    LED7 = (((BMS_Status_Flag3 & 0x01) == 1)); //充电枪插入
    //右转向灯
    if (IN5 || wake_up1)
        LED8 = FLASH;
    else LED8 = 0;

    LED9 = (ADV[3] < 2166); //气压1低信号

    LED10 = 1; //总电源指示   

    LED11 = (ADV[4] < 2166); //气压2低信号
...
/////////////////////////下车门铃
    ...
    if (/*(timer > 108 && timer <= 148) || (timer > 84 && timer <= 104) ||*/(timer[0] > 24 && timer[0] <= 64) || (timer[0] > 0 && timer[0] <= 20))
        buzz_run[0] = 1;
    else
        buzz_run[0] = 0;
    //////////////////////////////残疾人
    if(fKL10 && buzz_flag[1]){
        buzz_flag[1] = 0;
        timer[1] = 200;
    }else
        buzz_flag[1] = 1;
    ...
    //蜂鸣器 为0时响，为1时不响   
    if ((pSpeed > 69) || (DMC_Status == 3 && DMC_Level == 0) || buzz_run[0])
        BUZZ = 0;
    else if (!fKL4      //左前摩擦片
            || !fKL1    //左后摩擦片
            || !fKL5    //右前摩擦片
    ...
    if (Vcan_count == CAN_TIME) {
        DMC_Torque = 32000; //电机转矩
        DMC_Rpm = 32000; //电机转速
        DMC_Current = 32000; //电机控制器母线电流
        DMC_Controller_temp = 40; //电机控制器温度
        DMC_Moto_temp = 40; //电机温度
        DMC_Speed = 0; //车速
    }
}
...
void SYSTME_Logic(void) {
    ...
    M_PO1 = (LED15);                            //位置灯
    M_PO2 = (IN4 && IN24);                      //厢灯2
    M_PO3 = ((pSpeed < 3) && IN6 && IN4);       //前门开电磁阀
    M_PO4 = (IN7 && IN4);                       //前门关电磁阀
    M_PO5 = ((pSpeed < 3) && IN11 && IN4);      //中门开电磁阀
    M_PO6 = (IN12 && IN4);                      //中门关电磁阀
    M_PO7 = (LED4);                             //左转向灯
    ...
    M_PO14 = (IN4 || wake_up2);                 //A+信号输出
    M_PO15 = (IN4 && IN23);                     //厢灯1
    M_PO16 = 0;                                 //励磁电流

    R_PO1 = (M_ON);                             //电机电源
    R_PO2 = (IN4);                              //投币机电源     
    R_PO3 = (DMC_Gear == 2);                    //倒车信号      
    R_PO4 = (LED19);                            //左前雾灯
    R_PO5 = (M_ON);                             //集中润滑
    R_PO6 = (LED8);                             //右转向灯
    R_PO7 = (LED4);                             //左转向灯
    R_PO8 = (LED19);                            //右前雾灯
    R_PO9 = (LED24);                            //制动信号
    R_PO10 = (LED15 && LED1);                    //前门踏步灯
    //R_PO11 = (mKH1 && IN6); //前门踏步灯
    R_PO12 = (LED15 && LED2);                   //中门踏步灯
    //R_PO13 = (mKL15 && IN6); //中门踏步灯
    R_PO14 = (LED15);                           //位置灯
    R_PO15 = (IN4 || M_ON || rKH1);             //主继电器电源
    R_PO16 = 0;
...
void PCAN_DMC(void) {
    ...
    CAN_DATA.bit1 = (!LED1 && !fKL12); //前门状态
    CAN_DATA.bit2 = (LED1 || fKL12);
    CAN_DATA.bit3 = (!LED2 && !fKH1 && !fKL12); //中门状态和后舱门状态
    CAN_DATA.bit4 = (LED2 || fKH1 || fKL12);
    CAN_DATA.bit5 = IN27; //强制行车开关
    CAN_DATA.bit6 = LED18;//手刹
    CAN_DATA.bit7 = 1;
    CAN_DATA.bit8 = 1;
...
void PCAN_LCD_CANBUS1(void) {
    ...
    msg.data[1] = (unsigned char) (temp >> 8); //中八位
    msg.data[2] = (unsigned char) (temp >> 16); //高八位
    temp = pSpeed * 10;
...
void PCAN_SEND1(void) {
    ...
    CAN_DATA.bit1 = M_ON;            //钥匙ON
    CAN_DATA.bit2 = LED18;           //手刹
    CAN_DATA.bit3 = (DMC_Gear == 2); //倒车
    CAN_DATA.bit4 = 0;               //保留
    CAN_DATA.bit5 = 0;               //保留
    CAN_DATA.bit6 = 0;               //保留
    CAN_DATA.bit7 = fKH1;            //后舱门
    CAN_DATA.bit8 = 0;               //保留
    msg.data[4] = CAN_DATA.byte;

    CAN_DATA.bit1 = 0;                            //保留
    CAN_DATA.bit2 = LED16; //蓄电池电压低
    CAN_DATA.bit3 = ((Fcan_count == CAN_TIME) ||  //仪表系统故障
                    (Mcan_count == CAN_TIME)); 
    CAN_DATA.bit4 = LED2;                         //中门
...
void PCAN_Send_CarNet1(void) {
    ...
    CAN_DATA.bit1 = LED1;                               //门1
    CAN_DATA.bit2 = 0;
    CAN_DATA.bit3 = LED2;                               //门2
    CAN_DATA.bit4 = 0;
    CAN_DATA.bit5 = 1;                                  //门3
    CAN_DATA.bit6 = 1;
    CAN_DATA.bit7 = 1;                                  //门4
    ...
    CAN_DATA.bit1 = IN1;                                //左转向灯
    CAN_DATA.bit2 = 0;
    CAN_DATA.bit3 = IN5;                                //右转向灯
    CAN_DATA.bit4 = 0;
    CAN_DATA.bit5 = (DMC_Gear == 1);                    //前进挡
    CAN_DATA.bit6 = 0;
    CAN_DATA.bit7 = (DMC_Gear == 2);                    //倒车档
    CAN_DATA.bit8 = 0;
    msg.data[1] = CAN_DATA.byte;

    CAN_DATA.bit1 = ((DMC_Gear == 0) || (DMC_Gear == 3));//空挡
    CAN_DATA.bit2 = 0; 
    CAN_DATA.bit3 = LED24;           //刹车
    CAN_DATA.bit4 = 0;
    CAN_DATA.bit5 = LED18;           //手刹
    CAN_DATA.bit6 = 0;
    CAN_DATA.bit7 = LED17;           //远光灯
    CAN_DATA.bit8 = 0;
    msg.data[2] = CAN_DATA.byte;
    ...
    CAN_DATA.bit2 = 1;    
    CAN_DATA.bit3 = LED9;            //气压1低
    CAN_DATA.bit4 = 0;
    CAN_DATA.bit5 = LED11;           //气压2低
    CAN_DATA.bit6 = 0;
    CAN_DATA.bit7 = mKL6;            //水位报警
    CAN_DATA.bit8 = 0;
    msg.data[4] = CAN_DATA.byte;

    CAN_DATA.bit1 = (DCDC_Code > 0); //DCDC故障
    CAN_DATA.bit2 = 0;
    CAN_DATA.bit3 = 1;               //空滤报警
    CAN_DATA.bit4 = 1;
    CAN_DATA.bit5 = 1;               //机油压力报警   
    CAN_DATA.bit6 = 1;
    CAN_DATA.bit7 = 1;               //缓速器信号
    CAN_DATA.bit8 = 1;
    msg.data[5] = CAN_DATA.byte;

    CAN_DATA.bit1 = wake_up1;        //危险信号
    CAN_DATA.bit2 = 0;
    CAN_DATA.bit3 = IN4;             //ACC
    CAN_DATA.bit4 = 0;
    CAN_DATA.bit5 = M_ON;            //ON
    CAN_DATA.bit6 = 0;
    CAN_DATA.bit7 = rKH1;            //启动信号
    CAN_DATA.bit8 = 0;
    msg.data[6] = CAN_DATA.byte;

    CAN_DATA.bit1 = 1;              //充电状态
    CAN_DATA.bit2 = !LED6;
    CAN_DATA.bit3 = 0;     
    CAN_DATA.bit4 = 0;
    CAN_DATA.bit5 = 0;              //运行模式
    CAN_DATA.bit6 = 0;
    CAN_DATA.bit7 = 0;     
    CAN_DATA.bit8 = 0;
    msg.data[7] = CAN_DATA.byte;
...
void PCAN_Send_CarNet2(void) {
    ...
    msg.data[3] = pSpeed;//车速
    
    temp = e_total_miles * 10;
    msg.data[4] = (unsigned char) (temp); //低八位
    msg.data[5] = (unsigned char) (temp >> 8); //二级八位
    msg.data[6] = (unsigned char) (temp >> 16); //三级八位
    msg.data[7] = (unsigned char) (temp >> 24); //高八位
    msg.id = 0x18FF02E6;
    msg.len = 0x08;
    msg.type = 0x00;
    CAN2_SendMessage(&msg);
...
unsigned char Air_Set_t = 60; //设定温度
unsigned char Air_Power; //功率
...
unsigned char DC3positive_Temp = 40; //充电插头DC3+温度
unsigned char DC3negative_Temp = 40; //充电插头DC3-温度
unsigned char DC4positive_Temp = 40; //充电插头DC4+温度
unsigned char DC4negative_Temp = 40; //充电插头DC4-温度
unsigned char BMS_KtStatus_Flag5; //继电器状态5
...
void __attribute__((__interrupt__, auto_psv)) _T1Interrupt(void) {//10ms中断一次
    ...
    /*if (can1 == 0)//100ms循环发送报文
        ...
        PCAN_Send_TCO1();
    else if (can1 == 2 && (frmID == 6 || frmID == 7))//在电池界面时才发送电池
        PCAN_LCD_BMS();
    else if (can1 == 3)
        PCAN_Send_TCO2();
    else if (can1 == 4)
