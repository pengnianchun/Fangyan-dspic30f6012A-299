...
void LED_Logic(void) {
    ...
    //蜂鸣器 为0时响，为1时不响   
    if ((pSpeed > 109) || (DMC_Status == 3 && DMC_Level == 0))
        BUZZ = 0;
    else if (mKL4       //应急阀报警
            || rKL4     //应急阀报警
            || mKL6     //水位报警
...
void SYSTME_Logic(void) {
    ...
    F_PO11 = LED17; //右远光灯
    F_PO12 = wake_up2; //集中润滑电源 
    F_PO13 = LED22; //左近光
    F_PO14 = LED15; //位置灯
    F_PO15 = LED22; //右近光
    F_PO16 = 0; //励磁电流

    M_PO1 = (FLASH && (IN5 || wake_up1)); //右后转向灯
    M_PO2 = (wake_up2 && IN24); //厢灯2
    M_PO3 = (LED15 && rKH1); //前门踏步灯
    M_PO4 = (wake_up2 && rKL6); //制动灯
    M_PO5 = (LED15 && rKL15); //中门踏步灯 
    M_PO6 = (wake_up2 && IN7); //电视机电源
    M_PO7 = (wake_up2 && fKL10); //后视镜加热
    M_PO8 = (IN9 && IN10 && LED15); //后雾灯
    M_PO9 = (wake_up2 && IN19); //阅读灯
    M_PO10 = (FLASH && (IN1 || wake_up1)); //左后转向灯
    M_PO11 = (wake_up2 && LED15); //小灯
    ...
    R_PO1 = (wake_up2 && fKL11); //行李舱灯
    R_PO2 = fKL4; //前门电磁阀关                     
    R_PO3 = (LED15 && rKH1); //前门踏步灯       
    R_PO4 = wake_up2; //气压传感器电源
    R_PO5 = (LED15 && rKL15); //中门踏步灯
    R_PO6 = (FLASH && (IN5 || wake_up1)); //右侧转向灯
    R_PO7 = (FLASH && (IN1 || wake_up1)); //左侧转向灯
    R_PO8 = fKL6; //中门电磁阀开
    R_PO9 = (F3s_1 || F3s_2 || F3s_3);   //电控排污阀
    R_PO10 = fKL8; //中门电磁阀关
    R_PO11 = (wake_up2 && rKL6); //冷凝器
    //R_PO12 = (IN6); //位置灯示高灯
    R_PO13 = (wake_up2 && LED15); //侧标志灯
    R_PO14 = 1; //门泵电源
    R_PO15 = fKL2; //前门电磁阀开
    R_PO16 = 0;
...
void PCAN_DMC(void) {
    ...
    CAN_DATA.bit1 = !rKH1; //前门状态
    CAN_DATA.bit2 = rKH1;
    CAN_DATA.bit3 = (!rKL15 && !mKH1); //中门状态和后舱门状态
    CAN_DATA.bit4 = (rKL15 || mKH1);
    CAN_DATA.bit5 = IN27; //强制行车开关
    CAN_DATA.bit6 = rKL2; //手刹
    CAN_DATA.bit7 = 1;
    CAN_DATA.bit8 = 1;
...
void PCAN_SEND3(void) {
    ...
    CAN_DATA.bit1 = (rKL4 || mKL4); //门应急阀开关报警
    CAN_DATA.bit2 = 0;
...
void PCAN_Send_TCO2(void) {
    ...
    */
    ...
    CAN_DATA.bit1 = rKL2; //刹车工作
    CAN_DATA.bit2 = 0;
    CAN_DATA.bit3 = 0; //缓速器工作
    CAN_DATA.bit4 = 0;
    CAN_DATA.bit5 = rKH1; //门2开
    CAN_DATA.bit6 = 0;
    CAN_DATA.bit7 = rKL15; //门1开
    CAN_DATA.bit8 = 0;
    msg.data[5] = CAN_DATA.byte;
...
char Count_Time(unsigned char number, //输出序号
    ...
    if (condition) {
        temp[number] = temp[number] > 6000000 ? 6000000 : temp[number] + 1; //计数上限，防止溢出
    } else {
        temp[number] = 0;
    }

    if (temp[number] > 0 && temp[number] < out_time * 100) {
        return 1;
    } else if (!cycle_en) {
        return 0;
    } else if (temp[number] < (out_time + stop_time) * 100) {
        return 0;
    } else {
        temp[number] = 0;
        return 0;
...
void __attribute__((__interrupt__, auto_psv)) _T1Interrupt(void) {//10ms中断一次
    ...
    F3s_1 = Count_Time(2, count7 > 7000, 3, 0, 0);
    F3s_2 = Count_Time(3, IN28, 3, 0, 0);
    F3s_3 = Count_Time(4, wake_up2 || M_ON, 3, 0, 0);
