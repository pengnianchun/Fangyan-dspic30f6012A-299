...
void LED_Logic(void) {
    ...
    //蜂鸣器 为0时响，为1时不响   
    if ((pSpeed > 69) || mKL2 || fKL15)
        BUZZ = 0;
    else if (mKL4 || mKL6 || fKL9 ||fKL11 //IN30
            || LED13
            || LED14
            || CAN_0x18F81F27.READY
            )
        BUZZ = !FLASH_1s;
...
long DataWhrite(unsigned char*p, unsigned char n) {//低字节数据放高字节
    unsigned char i;
    unsigned long temp = 0;
    for (i = 0; i < n; i++) {
        temp <<= 8;
    ...
    if (C2RX0CONbits.RXFUL) { //数据有效
        ...
        switch (msg.id) {
            ...
            case 0x10F811A4://
                ...
                memcpy(&CAN_0x10F811A4, msg.data, 8);
                break;
                
            case 0x10F813A4://
                memcpy(&CAN_0x10F813A4, msg.data, 8);
                break;
