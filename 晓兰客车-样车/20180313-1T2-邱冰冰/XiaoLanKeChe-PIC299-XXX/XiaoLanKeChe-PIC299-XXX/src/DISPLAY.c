...
void disp_baojing(void) {
    ...
    if (!CAN_0x18F81F27.VCUFaultCode)
        color = GRAY;
    else if (CAN_0x18F81F27.VCUFaultCode <= 50)
        color = RED;
    else if (CAN_0x18F81F27.VCUFaultCode <= 100)
        color = ORANGE;
    else if (CAN_0x18F81F27.VCUFaultCode <= 150)
        color = YELLOW;
    else if (CAN_0x18F81F27.VCUFaultCode <= 200)
        color = WHITE;      
...
/******************************************************************************/    
    ...
    if (color_old[12] != color) {
        color_old[12] = color;
        //LCD_DrawImage1BPP(142 + 66 * 0, 74 - 10, Bmp_baojing[11], color, BLACK); //中门应急阀
    }
    ...
    if (color_old[13] != color) {
        color_old[13] = color;
        //LCD_DrawImage1BPP(142 + 66 * 1, 74 - 10, Bmp_baojing[11], color, BLACK); //中门应急阀
    }

    if (!CAN_0x0C109E27.KT) color = YELLOW;
    else color = GRAY;
    if (color_old[16] != color) {
        color_old[16] = color;
        LCD_DrawImage1BPP(142 + 66 * 2, 74 - 10, Bmp_baojing[12], color, BLACK); //高压接触器
    }

    if (CAN_0x18F81F27.READY == 1) color = GREEN;
    else color = GRAY;
    ...
    if (alarm_update_flag == 0) { //3S 更新一次 
        switch (al_index) {
            ...
            case 2:
                ...
                if (CAN_0x18F81F27.ChargeStatus == 2) {
          ...
          /*  case 17:
            ...
            case 29:
                al_index = 30;
                if (CAN_0x18F81F27.READY > 1 && CAN_0x18F81F27.READY < 6) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_k, YELLOW, BLACK); //Kx故障
                    if (CAN_0x18F81F27.READY == 2)
                        LCD_DrawImage1BPP(x + 11, y + 8, BMP_SZ_2[0], YELLOW, BLACK); //0
                    else if (CAN_0x18F81F27.READY == 3)
                        LCD_DrawImage1BPP(x + 11, y + 8, BMP_SZ_2[3], YELLOW, BLACK);
                    else if (CAN_0x18F81F27.READY == 4)
                        LCD_DrawImage1BPP(x + 11, y + 8, BMP_SZ_2[4], YELLOW, BLACK);
                    else if (CAN_0x18F81F27.READY == 5)
                        LCD_DrawImage1BPP(x + 11, y + 8, BMP_SZ_2[2], YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 22, y + 8, BMP_Guzhang, YELLOW, BLACK);
                    LCD_DrawRect(x + 22 + 42, y, 250, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                }
            default:
...
void updata_Dangwei(unsigned int x, unsigned int y) {
    ...
    //gear = DMC_Gear;
    if (DMC_Gear == 0) gear = 0;
    else if (DMC_Gear == 0x01) gear = 3;
    else if (DMC_Gear == 0x20) gear = 1;
    else if (DMC_Gear == 0x10) gear = 2;
    LCD_DrawImage1BPP(x, y, BMP_DANGWEI[gear], GREEN, BLACK); //档位更新   0N  1D  2R 3P
}
...
void updata_DCDL(unsigned int x, unsigned int y) {
    ...
    DCDL = CAN_0x10F8159E.BMS_A * 0.05;
    if (DCDL < 1600) {
        DCDL = 1600 - DCDL;
...
void frmMain(void) {
    if (f_index == 0) { //只更新一次
        ...
        f_index = 2;
        updata_DAT(700, 318 + 15 + 38, CAN_0x10F813A4.MotoTorque, 0.1, 2000, WHITE, 4, 0); //电机转矩
    } else if (f_index == 2) {
        f_index = 3;
        disp_baojing();
        print_SZ_1(305, 318 + 15 + 38, CAN_0x18F81B9E.BAT_Max_Cell_Voltage, 4, 3, WHITE); //单体高电压
        print_SZ_1(305, 318 + 15 + 38 * 2, CAN_0x18F8199E.BAT_Min_Cell_Voltage, 4, 3, WHITE); //单体低电压
    } else if (f_index == 3) {
        f_index = 4;
        disp_miles();
        //门开关图像报警
        ...
        updata_DAT(540 + 50, 318 + 15 + 38 * 2, CAN_0x10F811A4.MotorControllerTemperature, 1, 40, WHITE, 3, 0); //电机控制器温度
        updata_DAT(540 + 50, 318 + 15 + 38, CAN_0x10F811A4.MotorTemperature, 1, 40, WHITE, 3, 0); //电机温度
    } else if (f_index == 5) {
        f_index = 6;
        updata_SOC(585, 155 - 10);
        updata_DAT(150 + 45, 318 + 15 + 38, CAN_0x10F8159E.BMS_V * 0.1, 1, 0, WHITE, 4, 0); //总电压
        //updata_DAT(150 + 45, 318 + 15 + 38 * 2, BMS_A * 0.1, 1, 3200, WHITE, 4, 0); //总电流
        updata_DCDL(150 + 45 + 16, 318 + 15 + 38 * 2); //总电流
    } else if (f_index == 6) {
        f_index = 7;
        updata_DAT(410 + 50, 318 + 15 + 38, CAN_0x18F8189E.BAT_Max_Cell_Temperature, 1, 40, WHITE, 3, 0); //单体高温
        updata_DAT(410 + 50, 318 + 15 + 38 * 2, CAN_0x10F8179E.BAT_Min_Cell_Temperature, 1, 40, WHITE, 3, 0); //单体低温       
...
void ICFrame(unsigned char index) { //仪表框架
    ...
    if (index == 1) {
        ...
        //IN2
        //IN3  中门开状态
        LCD_DrawImageSBPP(35, 75 + 25 * 2, 21, ZMKZT, WHITE, BLACK);
        //IN4  前门开状态
...
void MOTOFrame(unsigned char index) {
    ...
    if (index == 1) {
        /*print_SZ(750, 50, 1, 1, 0, WHITE); // 1/3
...
void MOTOUpdate(unsigned char index) {
    ...
    if (index == 1) {
        ...
        if (CAN_0x10F813A4.MotoSpeed > 20000)
            temp = CAN_0x10F813A4.MotoSpeed - 20000;
        else temp = 20000 - CAN_0x10F813A4.MotoSpeed;
        if (temp <= 5) temp = 0;
...
void DMQFrame(void) {
    ...
    LCD_DrawImage1BPP(82 + 320, 100 + 2 + 38 * 4, BMP_Guzhang, WHITE, BLACK); //故障
    LCD_DrawImage1BPP(82 + 320 + 42, 100 + 2 + 38 * 4, BMP_Code, WHITE, BLACK); //代码
...
void DMYFrame(void) {
    ...
    LCD_DrawImage1BPP(82 + 320, 100 + 2 + 38 * 5, BMP_Guzhang, WHITE, BLACK); //故障
    LCD_DrawImage1BPP(82 + 320 + 42, 100 + 2 + 38 * 5, BMP_Code, WHITE, BLACK); //代码
}
//一直更新的数据
void DMYUpdate(void) {
...
void BMSFrame(unsigned char index) {
    ...
    if (index == 1) {
        ...
        /*LCD_DrawImage1BPP(402, 366, BMP_Zheng, WHITE, BLACK); //正
        LCD_DrawImage1BPP(423, 366, BMP_Jueyuan, WHITE, BLACK); //绝缘
        LCD_DrawImage1BPP(465, 366, BMP_DC, WHITE, BLACK); //电池
        LCD_DrawImage1BPP(486, 366, BMP_Zu, WHITE, BLACK); //阻
        LCD_DrawImage1BPP(509, 371, BMP_Maohao, WHITE, BLACK); //:
        LCD_DrawImage1BPP(752, 366, BMP_k, WHITE, BLACK); //K
        LCD_DrawImage1BPP(765, 368, BMP_Omu, WHITE, BLACK);*/ //Ω
        //LCD_DrawImageSBPP(765, 368, WHITE, 21, BMP_Omu, 2, BMP_Omu, 2, BMP_Omu, 2, BMP_Omu, 2, BMP_Omu);
...
void BatteryUpdate(unsigned char index) {
    ...
    if (index == 1) {
        for (j = 0; j < 5; j++) {
            for (i = 0; i < 10; i++) {
                if (BATCellVoltage[j * 10 + i] == 0)
                    print_SZ_1(60 + j * 160, 46 + 70 + 36 * i, BATCellVoltage[j * 10 + i], 4, 3, RED);
                else print_SZ_1(60 + j * 160, 46 + 70 + 36 * i, BATCellVoltage[j * 10 + i], 4, 3, GREEN); //单体电压
            }
        }
    }
    ...
    if (index == 2) {
        for (j = 0; j < 5; j++) {
            for (i = 0; i < 10; i++) {
                if (BATCellVoltage[50 + j * 10 + i] == 0)
                    print_SZ_1(60 + j * 160, 46 + 70 + 36 * i, BATCellVoltage[50 + j * 10 + i], 4, 3, RED);
                else print_SZ_1(60 + j * 160, 46 + 70 + 36 * i, BATCellVoltage[50 + j * 10 + i ], 4, 3, GREEN); //单体电压
            }
        }
    }
    ...
    if (index == 3) {
        for (j = 0; j < 5; j++) {
            for (i = 0; i < 10; i++) {
                if (BATCellVoltage[100 + j * 10 + i] == 0)
                    print_SZ_1(60 + j * 160, 46 + 70 + 36 * i, BATCellVoltage[100 + j * 10 + i], 4, 3, RED);
                else print_SZ_1(60 + j * 160, 46 + 70 + 36 * i, BATCellVoltage[100 + j * 10 + i], 4, 3, GREEN); //单体电压
            }
        }
    }
    ...
    if (index == 4) {
        for (j = 0; j < 5; j++) {
            for (i = 0; i < 10; i++) {
                if (BATCellVoltage[150 + j * 10 + i] == 0)
                    print_SZ_1(60 + j * 160, 46 + 70 + 36 * i, BATCellVoltage[150 + j * 10 + i], 4, 3, RED);
                else print_SZ_1(60 + j * 160, 46 + 70 + 36 * i, BATCellVoltage[150 + j * 10 + i], 4, 3, GREEN); //单体电压
            }
        }
    }
    ...
    if (index == 5) {
        for (j = 0; j < 5; j++) {
            for (i = 0; i < 7; i++) {
                if (BATCellTemp[j + 1][i] == 0)
                    updata_DAT(60 + 23 + 42 + j * 160 - 40, 46 + 70 + 36 * i, BATCellTemp[j + 1][i], 1, 40, RED, 3, 0); //单体温度
                else updata_DAT(60 + 23 + 42 + j * 160 - 40, 46 + 70 + 36 * i, BATCellTemp[j + 1][i], 1, 40, GREEN, 3, 0); //单体温度
            }
        }
    }
    ...
    if (index == 6) {
        for (j = 0; j < 5; j++) {
            for (i = 0; i < 7; i++) {
                if (BATCellTemp[j + 6][i] == 0)
                    updata_DAT(60 + 23 + 42 + j * 160 - 40, 46 + 70 + 36 * i, BATCellTemp[j + 6][i], 1, 40, RED, 3, 0); //单体温度
                else updata_DAT(60 + 23 + 42 + j * 160 - 40, 46 + 70 + 36 * i, BATCellTemp[j + 6][i], 1, 40, GREEN, 3, 0); //单体温度
            }
        }
    }
    /*for (i = 0; i < 4; i++) {
