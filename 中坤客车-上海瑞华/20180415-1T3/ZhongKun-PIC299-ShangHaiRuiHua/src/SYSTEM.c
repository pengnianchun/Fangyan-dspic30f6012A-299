...
void SYSTME_Logic(void) {
    ...
    F_PO11 = LED17; //右远光灯
  //F_PO12 = M_ON; //行车记录仪电源
    F_PO12 = ((pSpeed>2)&&(pSpeed<20));//pSpeed>0 && 低速低于10km ，外部输出到喇叭响，行人警示作用
    F_PO13 = (IN8); //左近光
    F_PO14 = (IN6); //位置灯
    F_PO15 = (IN8); //右近光
...
void LED_Logic(void) {
    ...
    if (pSpeed > 69)
        BUZZ = 0;
            ...
            //IN34 || //前应急阀1
              IN35 || //中应急阀1
             (IN36 && IN37 && (pSpeed > 5) && M_ON) ||  //安全带
            (!IN36 && !LED18 && READY && (VCU_Gear==13 || VCU_Gear==14) ) ||//重力感应 
             (pSpeed > 2 && pSpeed < 20)               //低速报警
            )
        BUZZ = !FLASH_1s;
