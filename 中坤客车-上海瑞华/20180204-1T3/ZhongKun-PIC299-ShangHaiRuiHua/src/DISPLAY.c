    ...
    if (alarm_update_flag == 0) { //3S 更新一次 
        ...
        switch (al_index) {
            ...
            case 31:
                al_index = 0;
                if (!IN17 && READY) {  
                    LCD_DrawImage1BPP(x, y + 8, Bmp_ZLBJ, YELLOW, BLACK); //重力报警 ZLBJ
...
void ICFrame(unsigned char index) { //仪表框架
    ...
    if (index == 1) {
        ...
        //安全带
        LCD_DrawImage1BPP(285, 75 + 25 * 7, Bmp_AQDai, WHITE, BLACK);     
