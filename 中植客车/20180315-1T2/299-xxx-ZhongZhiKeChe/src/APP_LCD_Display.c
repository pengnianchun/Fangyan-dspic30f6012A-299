...
typedef struct {         
    __prog__ const unsigned char **img;//报警文字
...
static void CAN_WARN_UPDATE(void) {
    ...
    list[40].position = 16 * 3 + 21 * 4;
    list[40].sum = 6;
    list[40].dat = CAN_0x18FECAEF.VCU_DM1.SPN;
...
static void CAN_WARN_DYNAMIC_UPDATE(CAN_WARN_LIST list, unsigned int x, unsigned int y) {
    if (list.condition)
...
void disp_BMP_baojing(void) {
    ...
    if (CAN_0x18EF5AEF.No18 || IN29) color = RED;
    else color = GRAY;
...
void updata_Dangwei(unsigned int x, unsigned int y) {
    ...
    //else if (CAN_0x18EF4AEF.EV_Gear == 1 || CAN_0x18EF4AEF.EV_Gear == 2) gear = 1;
    else if (CAN_0x18EF4AEF.EV_Gear == 13) gear = 2;
    else if (CAN_0x18EF4AEF.EV_Gear == 15) gear = 3;
    else gear = 0;
#if 1
    if(CAN_0x18EF4AEF.EV_Gear == 1)
        LCD_DrawImage1BPP(x, y, BMP_Gear1, GREEN, BLACK);
    else if (CAN_0x18EF4AEF.EV_Gear == 2)
        LCD_DrawImage1BPP(x, y, BMP_Gear2, GREEN, BLACK);
    else
#endif
...
void frmMain(void) {
    if (f_index == 0) { //只更新一次
        ...
        updata_DAT(150 + 45, 318 + 15 + 36 - 5, BMS_V, 0.2, 0, WHITE, 4, 0); //总电压
...
void BMSUpdate(unsigned char index) {
    if (index == 1) {
        print_SZ_1(82 + 200 + 30, 100 - 8, BMS_V * 0.2, 3, 0, WHITE);
        updata_DAT(82 + 200 + 30, 100 - 8 + 38, BMS_A, 0.1, 1000, WHITE, 3, 0);
        print_SZ_1(82 + 200 + 30, 100 - 8 + 38 * 2, BMS_SOC / 2, 3, 0, WHITE);
