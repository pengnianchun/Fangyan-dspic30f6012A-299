...
_CAN_0x18EF4EEF CAN_0x18EF4EEF;
_CAN_0x18EF4FEF CAN_0x18EF4FEF;
_CAN_0x18EF5AEF CAN_0x18EF5AEF;
_CAN_0x18EF5BEF CAN_0x18EF5BEF;
_CAN_0x18FECAEF CAN_0x18FECAEF;
...
void __attribute__((__interrupt__, auto_psv)) _C2Interrupt(void) {
    ...
    if (C2RX0CONbits.RXFUL) { //数据有效
        ...
        switch (msg.id) {  
            ...
            case 0x18EF4EEF:
                for(i = 0;i < 8;i++){
                    CAN_0x18EF4EEF.byte[i] = msg.data[i];
                }
                break;
                
            case 0x18EF4FEF:
                for(i = 0;i < 8;i++){
                    CAN_0x18EF4FEF.byte[i] = msg.data[i];
                }
                break;                
                
            case 0x18EF5AEF:
                for(i = 0;i < 8;i++){
                    CAN_0x18EF5AEF.byte[i] = msg.data[i];
                }              
                break;
                
            case 0x18EF5BEF:
                for(i = 0;i < 8;i++){
                    CAN_0x18EF5BEF.byte[i] = msg.data[i];
                }
                break;
                
            case 0x18FECAEF:
                CAN_0x18FECAEF.Ev_Fail_Mode = msg.data[0]&0x0F;
                CAN_0x18FECAEF.VCU_DM1.SPN = msg.data[2]
                        +((unsigned int) msg.data[3] << 8)
                        +((unsigned long) (msg.data[4]&0xE0) << 11);
                CAN_0x18FECAEF.VCU_DM1.FMI = (msg.data[4] & 0x1F);
                break;
            ...
            case 0x1805A89C:
                ...
                break;   
/************************************充电状态VCU不工作，采集BMS充电状态**************************************/                
            case 0x18FFA1F3:
                BMS_SOC = msg.data[2];
                break;

            case 0x18FFA2F3:
                BMS_V = ((unsigned char) msg.data[0] + (((unsigned int) msg.data[1] & 0x0F) << 8));
                BMS_A = ((unsigned char) msg.data[3] + ((unsigned int) msg.data[4] << 8));
                break;
                
            case 0x18FFAFF3:
                No27 = msg.data[0] & 0x03;
                ChargeStatus = (msg.data[0] >> 2) & 0x0F;
                break;
...
void CAN_DATA_Init(void){
    BMS_A = 1000;
    CAN_0x18EF4DEF.BAT_Max_Cell_Temperature = 40;
    CAN_0x18EF4DEF.BAT_Min_Cell_Temperature = 40;
    CAN_0x18EF4EEF.MCU_RPM = 20000;
    CAN_0x18EF4EEF.MotorTorque = 20000;
    CAN_0x18EF4EEF.OutSpeed = 20000;
    CAN_0x18EF4FEF.MCUCurrent = 1000;
    CAN_0x18EF4FEF.MCU_Control_Temp = 40;
    CAN_0x18EF4FEF.MCU_Temp = 40;   
}