...
void PCAN_SEND1(void) {
    ...
    msg.data[0] = ADV[0] / 2; //低压电池
    
    CAN_DATA.bit1 = LED2;            //中门状态
    CAN_DATA.bit2 = LED1;            //前门状态
    CAN_DATA.bit3 = 0;               //后舱门状态
    CAN_DATA.bit4 = M_ON;            //ON档信号
    CAN_DATA.bit5 = 0;               //START档信号
    CAN_DATA.bit6 = LED15;           //小灯状态
    CAN_DATA.bit7 = LED4;            //左转向灯
    CAN_DATA.bit8 = LED8;            //右转向灯
    msg.data[1] = CAN_DATA.byte;
    
    CAN_DATA.bit1 = LED9;            //前气压报警
    CAN_DATA.bit2 = LED11;           //后气压报警
    CAN_DATA.bit3 = LED18;           //手刹状态
    CAN_DATA.bit4 = 0;               //STOP信号
    CAN_DATA.bit5 = LED24;           //刹车信号
    CAN_DATA.bit6 = 0;               //
    CAN_DATA.bit7 = 0;               //
    CAN_DATA.bit8 = 0;               //
...
/*void PCAN_SEND3(void) {
    ...
    CAN_DATA.bit2 = 0;
    CAN_DATA.bit3 = IN11; //前桥制动气压报警信号
    CAN_DATA.bit4 = 0;
    CAN_DATA.bit5 = 0;    //制动报警信号
    CAN_DATA.bit6 = 0;
    CAN_DATA.bit7 = fKL6; //制动灯信号
    CAN_DATA.bit8 = 0;
    msg.data[1] = CAN_DATA.byte;
...
void PCAN_Send_TCO2(void) {
    ...
    */
    ...
    CAN_DATA.bit1 = IN11; //前气压报警
    CAN_DATA.bit2 = 0;    //
    CAN_DATA.bit3 = IN12; //后气压报警
    CAN_DATA.bit4 = 0;
    CAN_DATA.bit5 = 0;
    CAN_DATA.bit6 = 0;
    CAN_DATA.bit7 = 0;
...
void __attribute__((__interrupt__, auto_psv)) _T1Interrupt(void) {//10ms中断一次
    ...
    if ((ADV[0] < 220) && !F_20s_flag) {
        M_PO11_count = 1000;
    }
    F_20s_flag = (ADV[0] < 220);
    if ((M_PO11_count > 0)) {
        M_PO11_count--;
    }
