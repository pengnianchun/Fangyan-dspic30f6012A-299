...
void System_Init(void) {
    ...
    while (1) {
        ...
        */    
        ...
        if ((K_ON && M_ON ) || wake_up2) break; // 如果是ON唤醒则正常退出
...
void SYSTME_Logic(void) {
 ...
 // F_PO3  = (IN21 && M_ON);                    //喷水电机
    F_PO4  = (wake_up3 && IN11 && IN22);                  //左前雾灯
    F_PO5  = (wake_up3 && IN11);                          //开关背光小灯  
    F_PO6  = (FLASH && ((wake_up3 && IN10) || wake_up1)); //右转向灯
    F_PO7  = (FLASH && ((wake_up3 && IN9)  || wake_up1)); //左转向灯
    F_PO8  = (wake_up3 && IN24);                          //喇叭
    F_PO9  = (wake_up3 && IN12 && !IN13);                 //远光灯      
    F_PO10 = (wake_up3 && IN11 && IN3);                   //前门踏步灯
    F_PO11 =  F_PO9;                                      //远光灯
    F_PO12 = (wake_up3 && IN11 && IN2);                   //中门踏步灯 
    F_PO13 = (wake_up3 && IN12 && IN13);                  //近光灯 
    F_PO14 = (wake_up3 && IN11);                          //位置灯
    F_PO15 =  0;                                          //
    F_PO16 =  0;                                          //励磁电流
    
    M_PO1  =  0;                                //
    M_PO2  = (F_PO6);                           //右后转向灯
    M_PO3  = (wake_up3 && IN23 && F_PO4);       //后雾灯
    M_PO4  = (wake_up3 && mKL3);                //刹车灯
    M_PO5  =  0;                                //
    M_PO6  = (wake_up3 && mKL5);                //后舱灯 
    M_PO7  =  0;                                //
    M_PO8  = (wake_up2);                        //充电信号输出
    M_PO9  =  0;                                //
    if(wake_up3 && (Tmax>80))M_PO10=1; //40度开 //电池仓风扇电源 
    if(wake_up3 && (Tmax<75))M_PO10=0; //35度关                             
    M_PO11 =  0;                                //
    M_PO12 =  0;                                //
    M_PO13 = (F_PO14);                          //小灯电源
    if(wake_up3 &&((VCU_Status[0]&0X38)==0X08))M_PO14=1;     //倒车灯
    else                                       M_PO14=0; 
    M_PO15 = (F_PO7);                           //左后转向灯
    M_PO16 =  0;                                //励磁电流
...
void __attribute__((__interrupt__, auto_psv)) _T1Interrupt(void) {//10ms中断一次
    ...
    if (cnt0 >= 4) {//50ms
        ...
        BCAN_send_mile();
        BCAN_SendCtl();
        if (M_ON_flag && ((K_ON && wake_up3) || wake_up2)) {//若放在main函数里，会导致闪烁频率异常wake_up3置0时，M_ON不会立马置0，因为存在消抖延时
            LED_Logic();
            LED_Out();
            SYSTME_Logic();
        }
    ...
    if (IN1) { //ON开关判断//消抖//M_ON在掉电的一瞬间会使上拉电源无效，而M_ON在断电会保持300ms，从而影响开关量采集
        if (cnt30 > 30) K_ON = 1;
        else cnt30++;
    } else {
        if (cnt30 < 1) K_ON = 0;
