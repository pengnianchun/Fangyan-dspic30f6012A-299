...
void System_Init(void) {
    ...
    while (1) {
        ...
        if (IN2) {
            //充电指示
            LED6 = (VCU_Charge > 0 || BAT_ChargeGun == 0x55);
            //充电插头连接
            LED7 = (BAT_ChargeGun == 0x55 || BAT_ChargeGun == 0xAA);
        } else {
            LED6 = 0;
            LED7 = 0;
        }
...
void LED_Logic(void) {
    ...
    //SOC报警
    LED12 = (BAT_Status3.bit5 || BAT_Status3.bit8 || BAT_Status4.bit1);

    //电机故障
    LED13 = (MCU_Status1.byte > 0); 

    //动力电池系统故障
    LED14 = (BAT_Warn_Level > 0); 
    ...
    //远光灯
    LED17 = (IN7 && wake_up2 && !IN8); 

    //手刹
    LED18 = (fKL5); 

    //前雾灯
    LED19 = (IN9 && IN6 && wake_up2); 

    //后雾灯
    LED20 = (IN10 && IN6 && wake_up2); 

    //LED21 = (VCU_Gear==2 );//倒车灯

    LED22 = (IN8 && IN6 && wake_up2); //近光灯

    //LED23 = (IN14 && IN31 && M_ON);//缓速器

    LED24 = (rKL8 && wake_up2); //刹车指示


    //蜂鸣器 为0时响，为1时不响   
    if (pSpeed > 69)
        BUZZ = 0;
    else if (rKL2       //电机水位低报警
            || !rKL4    //左后蹄片报警
            || !rKL6    //右后蹄片报警
            || !rKL10   //左前蹄片报警
            || !rKL12   //右前蹄片报警
...
*/
    ...
    R_PO1 = (M_ON || VCU_Charge > 0);           //冷却水泵
    R_PO2 = IN26;                               //中门关电磁阀
    R_PO3 = (IN23 && wake_up2);                 //厢灯1       
    R_PO4 = (IN20 && (pSpeed < 5));             //前门开电磁阀
    R_PO5 = (IN25 && wake_up2);                 //司机灯
    R_PO6 = IN28;                               //前门关电磁阀
    R_PO7 = LED20;                              //后雾灯
    R_PO8 = LED4;                               //左转向灯
    R_PO9 = LED24;                              //制动灯
    R_PO10 = LED8;                              //右转向灯
    R_PO11 = (M_ON && (VCU_Gear == 0x52));      //倒车灯
    R_PO12 = LED15;                             //位置灯示高灯
    R_PO13 = LED15;                             //顶示廓灯
    R_PO14 = (IN32 && wake_up2);                //厢灯2 
    R_PO15 = (IN27 && (pSpeed < 5));            //中门开电磁阀
...
void BCAN_SendCtl(void) {
    ...
    temp.bit1 = ((M_ON && wake_up3) || wake_up2);//ON
    temp.bit2 = IN14; //慢档      
    temp.bit3 = IN15; //间歇
    temp.bit4 = IN16; //快档
    temp.bit5 = IN21; //喷水        
...
void PCAN_ICU2VCU01(void) {
    ...
    unsigned int temp;
    DATA_BIT CAN_DATA;
    message_ICU2VCU01.id = 0x18FFF117;

    CAN_DATA.bit1 = fKL5;  //手制动 
    CAN_DATA.bit2 = 0;
    CAN_DATA.bit3 = rKH1;  //前门开 
    CAN_DATA.bit4 = 0;
    CAN_DATA.bit5 = rKL15; //中门开
    CAN_DATA.bit6 = 0;
    CAN_DATA.bit7 = rKL1;  //舱门
    CAN_DATA.bit8 = 0;
    message_ICU2VCU01.data[0] = CAN_DATA.byte;

    CAN_DATA.bit1 = rKL8; //脚刹 
    CAN_DATA.bit2 = 0;
    CAN_DATA.bit3 = ((ADV[3]) < 2000); //前气压;
    CAN_DATA.bit4 = ((ADV[4]) < 2000); //后气压
    CAN_DATA.bit5 = 0;
    CAN_DATA.bit6 = 0;
    CAN_DATA.bit7 = 0;
    CAN_DATA.bit8 = 0;
    ...
    message_ICU2VCU01.data[3] = (ADR[5] - 84.27) * 2.5;
    
    message_ICU2VCU01.data[4] = ADV[0] / 2;//蓄电池电压

    if (ADV[3] < 500) temp = 0;
    else if (ADV[3] < 3000) temp = (ADV[3] - 500) * 0.04;
    else temp = 100;
    message_ICU2VCU01.data[5] = temp; //前气压
    
    if (ADV[4] < 500) temp = 0;
    else if (ADV[4] < 3000) temp = (ADV[4] - 500) * 0.04;
    else temp = 100;
    message_ICU2VCU01.data[6] = temp; //后气压    
...
void PCAN_ICU2VCU02(void) {
    ...
    message_ICU2VCU02.id = 0x18FFF217;
    ...
    message_ICU2VCU02.data[4] = (unsigned char) (e_single_miles); //低八位;
    message_ICU2VCU02.data[5] = (unsigned char) (e_single_miles >> 8); //二级八位
...
//整车控制器VCU
//0x0C01F3EF 
DATA_DOUBLE   VCU_Status1;
unsigned char VCU_LIFE;
//0x18FF01EF
//三合一
DATA_DOUBLE   VCU_THREE;
unsigned char VCU_READY;
unsigned char VCU_Gear;
unsigned int  VCU_Speed;
unsigned char VCU_Speed_percent; //加速踏板开度
unsigned char VCU_Brake_percent; //制动踏板开度
//0x18FF02EF
DATA_DOUBLE   VCU_Status2;

//电机控制器MCU
//0x1801F0EF
unsigned char MCU_Status;//电机状态
unsigned char MCU_Controller_Temp = 40; //TM电机控制器温度
unsigned int  MCU_Voltage; //电机输出电压
unsigned int  MCU_Current = 10000; //电机直流电流
//0x1802F0EF
unsigned char MCU_Temp = 40; //电机温度
unsigned int  MCU_RPM = 20000; //电机转速
unsigned int  MCU_NM = 20000; //电机扭矩
unsigned int  MCU_Power = 200; //电机功率
DATA_BIT      MCU_Status1;//电机报警

//电池信息
//0x0C01EFF3
DATA_DOUBLE   BAT_Status1;
DATA_DOUBLE   BAT_Status2;
unsigned char BMS_SOC;
unsigned char BMS_Status;
unsigned int  BMS_Ohm;
unsigned char BMS_Life;
//0x18FFB0F3
unsigned int  BAT_Single_H_Case; //电池单体高电压箱号
unsigned int  BAT_Single_H_Number; //电池单体高电压位置
unsigned int  BAT_Single_L_Case; //电池单体低电压箱号
unsigned char BAT_Single_L_Number; //电池单体低电压位置
//0x18FFB1F3
unsigned int  BAT_Temp_H_Case; //电池单体高温度箱号
unsigned int  BAT_Temp_H_Number; //电池单体高温度序号
unsigned int  BAT_Temp_L_Case; //电池单体低温度箱号
unsigned int  BAT_Temp_L_Number; //电池单体低温度序号
//0x18FFB2F3
unsigned char BAT_Single_H; //电池单体高电压
unsigned char BAT_Single_L; //电池单体低电压
unsigned char BAT_Temp_H = 40; //电池单体最高温度
unsigned char BAT_Temp_L = 40; //电池单体最低温度
//0x18FF11F3
unsigned char BAT_Case; //当前箱号
unsigned char BAT_Case_Voltage_Number; //当前箱号电压采集数量
unsigned char BAT_Case_Temp_Number; //当前箱号温度采集数量
unsigned char BAT_Case_Voltage_H; //当前箱号电池单体最高电压
unsigned char BAT_Case_Voltage_L; //当前箱号电池单体最低电压
unsigned char BAT_Case_Temp_H = 40; //当前箱号电池单体最高温度
unsigned char BAT_Case_Temp_L = 40; //当前箱号电池单体最低温度
//0x18FF13F3
unsigned int  BAT_Voltage; //电池总电压
unsigned int  BAT_Current = 10000; //电池总电流
unsigned char BAT_Temp = 40; //电池平均温度
//0x18FF90F3
unsigned char BAT_Charge;
unsigned char BAT_DC_Positive = 40;
unsigned char BAT_DC_Negative = 40;
unsigned char BAT_ChargeGun;
unsigned char BAT_Warming;
//0x18FF91F3
unsigned char BAT_Warn_Level;
DATA_BIT      BAT_Status3;
DATA_BIT      BAT_Status4;

//0x1801A79A
unsigned long DCDC_Input_Voltage; 
unsigned char DCDC_Output_Voltage; 
unsigned long DCDC_Output_Current; 
unsigned char DCDC_Output_Temp;
DATA_BIT      DCDC_Warn; 

//0x1801A79B
unsigned long DCAC_Oil_Input_Voltage; 
unsigned long DCAC_Oil_Output_Voltage; 
unsigned long DCAC_Oil_Output_Current; 
unsigned char DCAC_Oil_Output_Temp;
DATA_BIT      DCAC_Oil_Warn; 

//0x1801A79D
unsigned long DCAC_Air_Input_Voltage; 
unsigned long DCAC_Air_Output_Voltage; 
unsigned long DCAC_Air_Output_Current; 
unsigned char DCAC_Air_Output_Temp;
DATA_BIT      DCAC_Air_Warn; 
...
void __attribute__((__interrupt__, auto_psv)) _C2Interrupt(void) {
    ...
    if (C2RX0CONbits.RXFUL) { //数据有效
        ...
        switch (msg.id) {
            case 0x0C01F3EF:
                Vcan_count = 0;
                BMS_Connecter = (unsigned char) msg.data[0] & 0x03;
                Defrost_EN = (unsigned char) msg.data[1]>>6;
                Air_EN = (unsigned char) msg.data[2]& 0x03;
                Theree_EN = (unsigned char) (msg.data[2] >> 2) & 0x03;
                VCU_LIFE = (unsigned char) msg.data[7];
                break;

            case 0x18FF01EF:
                Vcan_count = 0;
                VCU_READY = (unsigned char) (msg.data[0] >> 4 ) & 0x03;
                VCU_THREE.byte = (unsigned char) msg.data[1];
                VCU_Gear = (unsigned char) msg.data[2];
                VCU_Speed_percent = (unsigned char) msg.data[3];
                VCU_Brake_percent = (unsigned char) msg.data[4];
                VCU_Speed= (unsigned char) msg.data[5]+((unsigned int) msg.data[6] << 8);
                break;
            ...
            case 0x1801F0EF:
                ...
                MCU_Controller_Temp = msg.data[2];
                MCU_Voltage = (unsigned char) msg.data[3]+((unsigned int) msg.data[4] << 8);
                MCU_Current = (unsigned char) msg.data[5]+((unsigned int) msg.data[6] << 8);
                break;
            ...
            case 0x1802F0EF:
                ...
                MCU_NM = (unsigned char) msg.data[3]+((unsigned int) msg.data[4] << 8);
                MCU_Power = (unsigned char) msg.data[5]+((unsigned int) msg.data[6] << 8);
                MCU_Status1.byte = msg.data[7];
                break;

            case 0x0C01EFF3:
                Bcan_count = 0;
                BAT_Status1.byte = msg.data[0];
                BAT_Status2.byte = msg.data[1];
                BMS_SOC = (unsigned char) msg.data[2];
                BMS_Status = msg.data[4] & 0x0F;
                BMS_Ohm = (unsigned char) msg.data[5]+((unsigned int) msg.data[6] << 8);
                BMS_Life = msg.data[7] & 0x0F;
                break;

            case 0x18FFB0F3://BMS STATUS 3
                Bcan_count = 0;
                BAT_Single_H_Case = (unsigned char) msg.data[0]+((unsigned int) msg.data[1] << 8);
                BAT_Single_H_Number = (unsigned char) msg.data[2]+((unsigned int) msg.data[3] << 8);
                BAT_Single_L_Case = (unsigned char) msg.data[4]+((unsigned int) msg.data[5] << 8);
                BAT_Single_L_Number = (unsigned char) msg.data[6]+((unsigned int) msg.data[7] << 8);
                break;
                
            case 0x18FFB1F3:
                Bcan_count = 0;
                BAT_Temp_H_Case = (unsigned char) msg.data[0]+((unsigned int) msg.data[1] << 8);
                BAT_Temp_H_Number = (unsigned char) msg.data[2]+((unsigned int) msg.data[3] << 8);
                BAT_Temp_L_Case = (unsigned char) msg.data[4]+((unsigned int) msg.data[5] << 8);
                BAT_Temp_L_Number = (unsigned char) msg.data[6]+((unsigned int) msg.data[7] << 8);
                break;

            case 0x18FFB2F3:
                Bcan_count = 0;
                BAT_Single_H = msg.data[0];
                BAT_Single_L = msg.data[1];
                BAT_Temp_H = msg.data[2];
                BAT_Temp_L = msg.data[3];
                break;

            case 0x18FF13F3:
                Bcan_count = 0;
                BAT_Voltage = (unsigned char) msg.data[3]+((unsigned int) msg.data[4] << 8);
                BAT_Current = (unsigned char) msg.data[5]+((unsigned int) msg.data[6] << 8);
                BAT_Temp = msg.data[7];
                break;

            case 0x18FF90F3:
                Bcan_count = 0;
                BAT_Charge = msg.data[0];
                BAT_DC_Positive = msg.data[1];
                BAT_DC_Negative = msg.data[2];
                BAT_ChargeGun = msg.data[3];
                BAT_Warming = msg.data[4] & 0x03;
                break;

            case 0x18FF91F3:
                Bcan_count = 0;
                BAT_Warn_Level = msg.data[0];
                BAT_Status3.byte = msg.data[1];
                BAT_Status4.byte = msg.data[2];
                break; 
                
            case 0x18FF11F3:
                Bcan_count = 0;
                BAT_Case = msg.data[0];
                BAT_Case_Voltage_Number = msg.data[1];
                BAT_Case_Temp_Number = msg.data[2];
                BAT_Case_Voltage_H = msg.data[3];
                BAT_Case_Voltage_L = msg.data[4];
                BAT_Case_Temp_H = msg.data[5];
                BAT_Case_Temp_L = msg.data[6];
                break;     
            
            case 0x1801A79A:
                Bcan_count = 0;
                DCDC_Input_Voltage = (unsigned char) msg.data[0]+((unsigned int) msg.data[1] << 8);
                DCDC_Output_Voltage = msg.data[2];
                DCDC_Output_Current = (unsigned char) msg.data[3]+((unsigned int) msg.data[4] << 8);
                DCDC_Output_Temp = msg.data[5];
                DCDC_Warn.byte = msg.data[6];
                break;

            case 0x1801A79B:
                Bcan_count = 0;
                DCAC_Oil_Input_Voltage = (unsigned char) msg.data[0]+((unsigned int) msg.data[1] << 8);
                DCAC_Oil_Output_Voltage = (unsigned char) msg.data[2]+((unsigned int) msg.data[3] << 8);
                DCAC_Oil_Output_Current = (unsigned char) msg.data[4]+((unsigned int) msg.data[5] << 8);
                DCAC_Oil_Output_Temp = msg.data[6];
                DCAC_Oil_Warn.byte = msg.data[7];
                break;

            case 0x1801A79D:
                Bcan_count = 0;
                DCAC_Air_Input_Voltage = (unsigned char) msg.data[0]+((unsigned int) msg.data[1] << 8);
                DCAC_Air_Output_Voltage = (unsigned char) msg.data[2]+((unsigned int) msg.data[3] << 8);
                DCAC_Air_Output_Current = (unsigned char) msg.data[4]+((unsigned int) msg.data[5] << 8);
                DCAC_Air_Output_Temp = msg.data[6];
                DCAC_Air_Warn.byte = msg.data[7];
                break;
                        
            case 0x18FECAEF:// DM1 VCU
                for (i = 0; i < 6; i++) {
