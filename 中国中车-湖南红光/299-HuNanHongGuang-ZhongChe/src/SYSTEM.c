...
void LED_Logic(void) {
    ...
    //LED23 = (IN14 && IN31 && M_ON);//取力

    LED24 = ((Brake_Pedal || IN16) && M_ON); //刹车指示
    //倒车灯
    /*
    ...
    if ((Warn_level || (pSpeed > 69)) && M_ON)
        BUZZ = 0;
    else if (((IN31 && (Engine_Speed > 0)) || IN22 || IN23 || (TCU_warn == 1) ||
...
void PCAN_CCVS(void) {
    ...
    message_CCVS.data[0] = (unsigned char) (M_ON << 6)//ON档
            ...
            + (unsigned char) (IN9 << 4)//后门开信号
            + (unsigned char) (IN8 << 3)//前面开信号
            + (unsigned char) (0 << 2)//后舱门开信号
            + (unsigned char) (LED18 << 1)//驻车信号
            + (unsigned char) (LED11) ;//气压低报警信号
    message_CCVS.data[1] = (unsigned char) (LED4 << 7)//左转向
            + (unsigned char) (LED17 << 6)//远光灯
            + (unsigned char) (LED19 << 5)//前雾灯
            + (unsigned char) (LED8 << 4)//右转向
            + (unsigned char) (LED1 << 3)//油量报警
            + (unsigned char) (LED5 << 2)//发动机预热
            + (unsigned char) (LED22 << 1)//近光灯
            + (unsigned char) (LED20) ;//后雾灯
