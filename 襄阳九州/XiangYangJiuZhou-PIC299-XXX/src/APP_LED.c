...
void LED_Logic(void) {
    ...
    LED6 = (BMS_Charge == 1);
    //充电插头连接
    LED7 = BMS_Status1.bit1; 
    //右转向灯
    /*if ((IN5) || WAKE_Danger)
        LED8 = FLASH;
    else LED8 = 0;*/

    if (ADV[3] < 2500)//气压1低信号
        LED9 = 1;
    else
        LED9 = 0;
    ...
    //LED12 = (SOC_Low || SOC_High || Diff_SOC || SOC_JUMP);

    LED13 = VCU_Power_System_Status.bit8;//电机故障

    LED14 = (BMS_Fault_Level > 0);  //动力电池系统故障
