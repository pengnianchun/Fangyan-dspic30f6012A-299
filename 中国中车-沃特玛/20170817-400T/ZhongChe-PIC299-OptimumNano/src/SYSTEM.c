...
unsigned char Turn_left_right=0;//转向标志
...
void SYSTME_Logic(void) {
 ...
 // F_PO3  = (IN21 && M_ON);                    //喷水电机
  ...
  //R_PO16 =  0;                                //励磁电流  
    ...
    if((KEY_state&0x80)||fKL6)M_PO4 = 1;        //刹车灯
    else                      M_PO4 = 0;
...
void LED_Logic(void) {
    ...
    if(IN24||(KEY_state&0x80))LED24 = 1;
    else                      LED24 = 0;  //刹车指示      
    if((DR_state&0x0F)==0X0D) LED21 = 1;
    else                      LED21 = 0;  //倒车灯
           ...
           ( //!fKL4 ||   //左前摩擦片报警
             ...
             //!fKL9 ||   //右后摩擦片报警
               ...
               !IN21 ||   //右前摩擦片报警
                IN16 ||   //缓速器高温报警
                IN30 ||   //安全带报警
              ((IN31==0)&&((system_state&0x30)==0x00))||   //重力感应报警
...
void PCAN_send_yb(void) {
    ...
    temp.bit0 =(IN22||IN23); //气压低报警 (LED9 || LED11)
    temp.bit1 = IN17; //驻车信号 LED18
    temp.bit2 = IN10; //后仓门
    temp.bit3 = IN8;  //前门开   LED1
    temp.bit4 = IN9;  //后门开   LED2
    temp.bit5 = M_ON; //二挡
    temp.bit6 = M_ON; //一档
    temp.bit7 = 0;
    msg.data[0] =temp.byte;
    
    temp.bit0 = IN19;  //后雾灯   LED20
    temp.bit1 = IN6;   //近光灯   LED22
    temp.bit2 = 0;     //发动机预热
    temp.bit3 = 0;     //油量报警
    temp.bit4 = IN3;   //右转向灯 LED8
    temp.bit5 = IN18;  //前雾灯   LED19
    temp.bit6 = IN7;   //远光灯   LED17
    temp.bit7 = IN4;   //左转向灯 LED4
...
void __attribute__((__interrupt__, auto_psv)) _T1Interrupt(void) {//10ms中断一次
    ...
    if (cnt1 >= 9) {//100ms
      ...
      //else Mcan_count++;
        ...
        //else Ycan_count++;
        if(IN3||IN4)Turn_left_right=10;
        if(Turn_left_right>0)Turn_left_right--;
