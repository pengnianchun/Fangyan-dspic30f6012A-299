...
//OFF?{（气压1&气压2）＞8.1个压}or(PT1000采集温度＞90℃）
    ...
    R_PO1  = (M_ON);                            //ON档输出电源
    R_PO3  = (M_ON);                            //ON档                             
    R_PO5  = (M_ON);                            //车载监控ON档电源
    R_PO6  = (IN4);                             //ST档电源
    R_PO7  = (M_ON);                            //ON档
    R_PO8  = (M_ON);                            //干燥器电源
    if(Gear==124)R_PO9=1;else R_PO9=0;          //倒车灯 
    R_PO10 = (M_ON);                            //空调ON档电源
    R_PO11 = (IN6 && IN9 && IN10);              //后雾灯                            
...
//蜂鸣器 为0时响，为1时不响   
        if (M_ON && (
             ...
             //!fKL5 ||   //右前摩擦片报警
                LED9 ||   //气压1
                LED11||   //气压2
               (LED1 && (Vspeed > 50))  ||   //前门开指示
               (LED2 && (Vspeed > 50))  ||   //中门开指示 
                LED4  ||         //左转向灯报警 
                LED8  ||         //右转向灯报警
               (Vspeed > 690)    //超速报警                
...
void PCAN_send_test(void) {
    ...
    msg.data[6] = (unsigned char)(ADR[1]);
    msg.data[7] = (unsigned char)(ADR[1]>>8);    
...
unsigned char BMU_NUM=6;   //定义模块数量
