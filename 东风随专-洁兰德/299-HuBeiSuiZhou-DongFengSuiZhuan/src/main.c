...
int main(void) {
    ...
    while (1) {
        ...
        if (M_ON) {// ON 闭合
            ...
            PWR_CTRL = 1;
            bLCD_RESET = 1;
            if(F100ms){//放在中断里会影响开机程序
              F100ms=0;
              LED_Logic();
              LED_Out();
              SYSTME_Logic();
            }
            pSpeed = (unsigned int) (Speed_rmp * 0.9144 * 60 * 3.14159 / 1000 / 5.286);
            /*if (pSpeed >= 70 && pSpeed < 100) Speed_lim_mode = 0x00; */
            if (pSpeed >= 100) Speed_lim_mode = 0x01;//限速模式
            else Speed_lim_mode = 0x03;
            sm_new_value[0] = pSpeed * 132 / 7;
            SM_Update_0(); //车速表
            sm_new_value[5] = (unsigned int) (EngineSpeed * 0.66 * 0.125);
            SM_Update_5(); //转速表   
