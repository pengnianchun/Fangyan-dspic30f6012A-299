...
void disp_BMP_baojing(void) {
    ...
    if (CAN_0x113.VCU_Flt > 0) color = RED;
    else color = GRAY;
    ...
    if (CAN_0x217.BMS_HigVolBatIsoSts == 2) color = RED;
    else color = GRAY;
    ...
    if (CAN_0x217.BMS_NegRlySts != 1 || CAN_0x217.BMS_PosRlySts != 1) 
        color = YELLOW;
    else 
        color = GRAY;
    ...
    if (CAN_0x23D.MCU_MtrCntrlTmpWarn || CAN_0x23D.MCU_MtrTmpWarn) color = RED;
    else color = GRAY;
    ...
    if (CAN_0x23D.MCU_VehRdy) color = GREEN;
    else color = GRAY;
...
void disp_FONT_baojing(unsigned int x, unsigned int y) {
    ...
    if (alarm_update_flag == 0) { //3S 更新一次 
        switch (al_index) {
            ...
            case 3:
                al_index = 4;
                if (CAN_0x213.BMS_LstComWithVCU || CAN_0x213.BMS_LstCommwithMCU) {
            ...
            case 6:
                al_index = 7;
                if (CAN_0x213.BMS_BatCellChrgAct) {
            ...
            case 9:
                al_index = 10;
                if (CAN_0x213.BMS_BatCellDischAct) {
            ...
            case 12:
                al_index = 13;
                if (CAN_0x213.BMS_BatDischOvCrnt) {
            ...
            case 13:
                al_index = 14;
                if (CAN_0x213.BMS_ChrgOvCrntFlt) {
            ...
            case 17:
                al_index = 18;
                if (CAN_0x219.BMS_SOCOverHighWarning) {
            ...
            case 25:
                al_index = 26;
                if (CAN_0x219.BMS_LstCommWithCCS) {
            ...
            case 26:
                al_index = 27;
                if (CAN_0x7E0.VCU_BusVoltage_Alarm == 1) {
                    LCD_DrawImageSBPP(x, y + 8, 42, DCDLGD, YELLOW, BLACK); //电池电量过低
                    LCD_DrawRect(x + 42 + 42 + 42, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                }
            ...
            case 28:
                ...
                if (CAN_0x7E0.VCU_BusVoltage_Alarm == 2) {
                    LCD_DrawImageSBPP(x, y + 8, 42, DCDLBZQTC, YELLOW, BLACK); //电池电量不足请停车
                    LCD_DrawRect(x + 42 + 42 + 42 + 63, y, 50, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                }
            ...
            case 31:
                al_index = 32;
                if (CAN_0x219.BMS_SOCJumpWarning) {
            ...
            case 36:
                ...
                if (CAN_0x217.BMS_HigVolBatIsoSts == 2) {
            ...
            case 37:
                al_index = 38;               
                if (CAN_0x113.VCU_CntrlSelfErrCodRsvrd > 0){
                    ...
                    print_SZ_2(x + 11 + 11 + 11 + 42 + 42 + 42, y + 8, CAN_0x113.VCU_CntrlSelfErrCodRsvrd, 3, 0, RED);
            ...
            case 38:
                al_index = 39;
                if (CAN_0x207.BMS_HVInterLockStsWrningRsvrd) {
            ...
            case 40:
                al_index = 41;
                if ((CAN_0x141.VCU_VCUAndMtrCtrlSelfErrCd > 0)) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_m, YELLOW, BLACK); //MCU系统故障xxx
                    LCD_DrawImage1BPP(x + 11, y + 8, BMP_c, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 11 + 11, y + 8, BMP_u, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 11 + 11 + 11, y + 8, BMP_System, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 11 + 11 + 11 + 42, y + 8, BMP_Guzhang, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 11 + 11 + 11 + 42 + 42, y + 8, BMP_Code, YELLOW, BLACK);
                    print_SZ_2(x + 11 + 11 + 11 + 42 + 42 + 42, y + 8, CAN_0x141.VCU_VCUAndMtrCtrlSelfErrCd, 3, 0, RED);
                    LCD_DrawRect(x + 11 + 11 + 11 + 42 + 42 + 42 + 48, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                }
            ...
            case 47:
                ...
                if (CAN_0x207.BMS_BatPckSysFltCod > 0) {
                    LCD_DrawImage1BPP(x, y + 8, BMP_b, YELLOW, BLACK); //BMS系统故障xxx
                    LCD_DrawImage1BPP(x + 11, y + 8, BMP_m, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 11 + 11, y + 8, BMP_s, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 11 + 11 + 11, y + 8, BMP_System, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 11 + 11 + 11 + 42, y + 8, BMP_Guzhang, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 11 + 11 + 11 + 42 + 42, y + 8, BMP_Code, YELLOW, BLACK);
                    print_SZ_2(x + 11 + 11 + 11 + 42 + 42 + 42, y + 8, CAN_0x207.BMS_BatPckSysFltCod, 3, 0, RED);
                    LCD_DrawRect(x + 11 + 11 + 11 + 42 + 42 + 42 + 48, y, 100, 36, BLACK);
                    alarm_update_flag = 1;
                    break;
                }
...
void updata_Dangwei(unsigned int x, unsigned int y) {
    ...
    /*gear = CAN_0x141.VCU_ShftLvlPos;
    if (CAN_0x7E0.VCU_Gear_R) gear = 2;
    else if (CAN_0x7E0.VCU_Gear_N) gear = 0;
    else if (CAN_0x7E0.VCU_Gear_D) gear = 1;
    else gear = 0;
    LCD_DrawImage1BPP(x, y, BMP_DANGWEI[gear], GREEN, BLACK); //档位更新   0是N ;1是D; 2是R ; 3是P*/
    
    if (CAN_0x7E0.VCU_Gear_Idle)//齿轮档位
        LCD_DrawImage1BPP(x + 56 - 66 * 4, y + 10, BMP_KONGDANG, GREEN, BLACK);
    else if (CAN_0x7E0.VCU_Gear_LowSpeed)
        LCD_DrawImage1BPP(x + 56 - 66 * 4, y + 10, BMP_DIDANG, GREEN, BLACK);
    else if (CAN_0x7E0.VCU_Gear_HighSpeed)
        LCD_DrawImage1BPP(x + 56 - 66 * 4, y + 10, BMP_GAODANG, GREEN, BLACK);
    else
        LCD_DrawImage1BPP(x + 56 - 66 * 4, y + 10, BMP_KONGDANG, GRAY, BLACK);
    
    if (CAN_0x7E0.VCU_Gear_D)//档位
        LCD_DrawImage1BPP(x + 56 - 66 * 1, y + 10, BMP_QIANJIN, GREEN, BLACK);
    else if (CAN_0x7E0.VCU_Gear_N)
        LCD_DrawImage1BPP(x + 56 - 66 * 1, y + 10, BMP_KONGDANG, GREEN, BLACK);
    else if (CAN_0x7E0.VCU_Gear_R)
        LCD_DrawImage1BPP(x + 56 - 66 * 1, y + 10, BMP_HOUTUI, GREEN, BLACK);
    else
        LCD_DrawImage1BPP(x + 56 - 66 * 1, y + 10, BMP_KONGDANG, GRAY, BLACK);
}

void updata_SOC(unsigned int x, unsigned int y) {
    unsigned char color = WHITE;
    static unsigned char soc = 100;
    soc = CAN_0x213.BMS_HVBatSOC;
...
void updata_DCDY(unsigned int x, unsigned int y)    {
    ...
    DCDY = CAN_0x213.BMS_HVBatProVol * 0.5;
...
void updata_DCDL(unsigned int x, unsigned int y) {
    ...
    DCDL = CAN_0x213.BMS_BatPckCrnt * 0.05;
    if (DCDL < 400) {
        DCDL = 400 - DCDL;
...
void mainFrame(void) {
    ...
    LCD_DrawImageSBPP(225 + 83 + 83 + 2, 365 - 50 + 10, 42, DJDL, WHITE, BLACK); //电机电流
    LCD_DrawImage1BPP(615 - 34 + 22, 365 - 50 + 10, BMP_a, WHITE, BLACK); //RPM
    ...
    LCD_DrawImageSBPP(225 + 83 + 83 + 2, 365 - 50 + 10 + 38 * 2, 42, DJDY, WHITE, BLACK); //电机电压
    LCD_DrawImage1BPP(615 - 22 + 11, 365 - 50 + 10 + 38 * 2, BMP_v, WHITE, BLACK); //
    ...
    LCD_DrawRect(225 + 83 + 83, 365 - 50, 1, 65 + 50, GREEN);
...
void frmMain(void) {
    if (f_index == 0) { //只更新一次
        ...
        f_index = 2;
        print_SZ_1(60, 318 + 38, CAN_0x215.BMS_BatPckCellMaxVol * 5, 4, 3, WHITE); //单体高
        print_SZ_1(60, 318 + 38 * 2, CAN_0x215.BMS_BatPckCellMinVol *5, 4, 3, WHITE); //单体低
        updata_DAT(313, 318 + 38, CAN_0x239.MCU_MtrTmp, 1, 40, WHITE, 3, 0); //电机温度
    } else if (f_index == 2) {
        f_index = 3;
        disp_BMP_baojing();
        updata_DAT(480 + 30, 318, CAN_0x23D.MCU_MtrCntrlRatCrnt, 0.05, 1000, WHITE, 4, 0); //电机电流
        updata_DAT(480 + 30, 318 + 38, CAN_0x239.MCU_MtrDlvrdTrq, 1, 510, WHITE, 4, 0); //电机扭矩
    } else if (f_index == 3) {
        f_index = 4;
        disp_miles();
        ...
        updata_DAT(313, 318, CAN_0x239.MCU_MtrCntrlTmp, 1, 40, WHITE, 3, 0); //电机控制器温度
    } else if (f_index == 5) {
        f_index = 6;
        updata_SOC(585, 155 + 60 + 60);
        updata_DAT(800 - 150, 318 + 38, CAN_0x21A.BMS_BatPckMaxTmp, 1, 40, WHITE, 3, 0); //单体高温
        updata_DAT(800 - 150, 318 + 38 * 2, CAN_0x21A.BMS_BatPckMinTmp, 1, 40, WHITE, 3, 0); //单体低温
    } else if (f_index == 6) {
        f_index = 7;
        updata_DCDY(585, 155);
        ...
        print_SZ_1(480 + 30, 318 + 38 * 2, CAN_0x23D.MCU_MtrInverRatVol * 0.5, 4, 0, WHITE); //电机电压
    ...
    switch (VIDEO) {
        ...
        case 1:
            ...
            //倒车视屏选择使能按键
            if ((CAN_0x141.VCU_ShftLvlPos == 1) && !R_flag) {//倒挡
                key_AV = 1; //模拟AV开关按一次
                AV_CH = 0; //0+1=1  倒车
                R_flag = 1; //防止一直进入这个函数，导致key_AV总是置1
            }
            if ((CAN_0x141.VCU_ShftLvlPos != 1) && R_flag) {
                key_AV = 1;
                AV_CH = 4; // 0 返回MCU界面
                R_flag = 0;
                M_flag = 0;
...
void update_setDATE(void) {
    ...
    temp = (unsigned int) (0 / 2 * new_pluse / 100);
    //车速
