...
x=433-40+9;y=280-9;
y=280-9+1*30;
   lcd_show_9dot(x,       y, Bmp_main_interface_2_R_1, WHITE,0x0000);//油量
...
//if((DR_state&0X80))      color=RED;//正逻辑 
  ...
  if(KT_Status[0]&0X02)color=GREEN;
  else              color=GRAY; 
...
void disp_baojing(void){
  ...
  LCD_DrawImage1BPP(275-18,25-18,Bmp_RY,color,BLACK);//油量报警
...
//第一列
  ...
  LCD_DrawImageSBPP(x,y+5*34,Bmp_FKL6, WHITE,BLACK);
...
//第二列 
  ...
  unsigned int Iid1[20]={0,0,0,0,0,136,0,0,0,160,
...
//第一列
  ...
  LCD_DrawImageSBPP(x,y+6*34,Bmp_RKL7, WHITE,BLACK);
  LCD_DrawImage1BPP(x,y+7*34,Bmp_RKL8, WHITE,BLACK);
  LCD_DrawImageSBPP(x,y+8*34,Bmp_RKL9, WHITE,BLACK);
    ...
    if (alarm_update_flag == 0) { //3S 更新一次 
        ...
        switch (al_index) {
            ...
            case 28: 
                ...
                //if(1){
                if(  M_ON  && !IN24  &&              //
                     ((system_state&0x30)==0x00)     //ready    
                ){ //离座前请断开高压    
                LCD_DrawImageSBPP(x,    y, Bmp_Display_bj_28,  Fcolor,color);            
                w=x+160;LCD_DrawRect(w, y, 250-w, 36, color);
                alarm_update_flag = 1;
                break; 
                }   
