...
void disp_BMP_baojing(void) {
    ...
    if (color_old[6] != color) {
        color_old[6] = color;
        LCD_DrawImage1BPP(8 + 66 * 6, 10, Bmp_baojing[8], color, BLACK); //门铃
    }
    ...
    if (VCU_BUSStatus == 2) color = GREEN;
    else color = GRAY;
    ...
    if (color_old[17] != color) {
        color_old[17] = color;
        LCD_DrawImage1BPP(140 + 66 * 5, 74 - 5, Bmp_baojing[14], color, BLACK); //安全带
...
void disp_FONT_baojing(unsigned int x, unsigned int y) {
    ...
    if (alarm_update_flag == 0) { //3S 更新一次 
        switch (al_index) {
            ...
            case 42:
                al_index = 43;
                /*if (VCU_FaultCode >= 1 && VCU_FaultCode <= 20) {//整车变速箱故障
                    LCD_DrawImage1BPP(x + 42, y + 8, BMP_Chesu, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x + 21, y + 8, BMP_Tiaobian, YELLOW, BLACK);
                    LCD_DrawImage1BPP(x, y + 8, BMP_Zhengche, YELLOW, BLACK);                                       
                    LCD_DrawImage1BPP(x + 84, y + 8, BMP_Xiang_BMS, YELLOW, BLACK);
...
void ICFrame(unsigned char index) { //仪表框架
    ...
    if (index == 1) {
        ...
        //IN1
        //IN2  后舱门
        LCD_DrawImage1BPP(35, 75 + 25 * 1, BMP_IC_Hou, WHITE, BLACK);
        LCD_DrawImage1BPP(35 + 21, 75 + 25 * 1, BMP_Cangmen, WHITE, BLACK);
        LCD_DrawImage1BPP(35 + 63, 75 + 25 * 1, BMP_IC_Kaiguan, WHITE, BLACK);
        //IN3  后门开信号
        ...
        /*************************第二列开关***********************************/
        //IN17 水位报警
        LCD_DrawImage1BPP(285, 75 + 25 * 0, BMP_Shuiwei, WHITE, BLACK);
        LCD_DrawImage1BPP(285 + 42, 75 + 25 * 0, BMP_Baojing, WHITE, BLACK);
        //IN18 驻车制动开关
        LCD_DrawImageSBPP(285, 75 + 25 * 1, 42, ZCZDKG, WHITE, BLACK);
        //IN19 门铃开关
        LCD_DrawImageSBPP(285, 75 + 25 * 2, 21, MLKG, WHITE, BLACK);
        //IN20 烟雾报警开关
        LCD_DrawImageSBPP(285, 75 + 25 * 3, 42, YWBJKG, WHITE, BLACK);
        //IN21
        //IN22
        //IN23 ABS故障
        LCD_DrawImageSBPP(285, 75 + 25 * 6, 11, ABS, WHITE, BLACK);
        //IN24 ASR故障
        LCD_DrawImageSBPP(285, 75 + 25 * 7, 11, ASR, WHITE, BLACK);
        //IN25 安全带报警
        LCD_DrawImageSBPP(285, 75 + 25 * 8, 42, AQDBJ, WHITE, BLACK);
        //IN26 应急阀报警
        LCD_DrawImageSBPP(285, 75 + 25 * 9, 42, YJFBJ, WHITE, BLACK);
        //IN27 左前摩擦片报警
        LCD_DrawImage1BPP(285, 75 + 25 * 10, BMP_IC_You, WHITE, BLACK);
        LCD_DrawImage1BPP(285 + 21, 75 + 25 * 10, BMP_IC_Qian, WHITE, BLACK);
        LCD_DrawImage1BPP(285 + 42, 75 + 25 * 10, BMP_Mocapian, WHITE, BLACK);
        LCD_DrawImage1BPP(285 + 105, 75 + 25 * 10, BMP_Baojing, WHITE, BLACK);
        //IN28 右前摩擦片报警
        LCD_DrawImage1BPP(285, 75 + 25 * 11, BMP_IC_Zuo, WHITE, BLACK);
        LCD_DrawImage1BPP(285 + 21, 75 + 25 * 11, BMP_IC_Qian, WHITE, BLACK);
        LCD_DrawImage1BPP(285 + 42, 75 + 25 * 11, BMP_Mocapian, WHITE, BLACK);
        LCD_DrawImage1BPP(285 + 105, 75 + 25 * 11, BMP_Baojing, WHITE, BLACK);
        //IN29
        //IN30
        //IN31
        //IN32
...
void BMSUpdate(unsigned char index) {
    ...
    if (index == 1) {
        ...
        print_SZ_1(82 + 200 + 30 + 16, 100 - 8 + 38 * 4, BMS_Life, 3, 0, WHITE);
