...
DATA_BIT      VCU_Fault[3];     
unsigned char Charging;              //充放电状态
unsigned char BMS_SOC;               //电池SOC
unsigned int  MCU_RPM = 3000;       //电机转速
unsigned int  BMS_V;                 //电池电压
unsigned int  BMS_A = 1000;         //电池电流

//整车控制器2 ID：0X1100EF1D
unsigned int  BMS_Charge_V;          //充电需求电压
unsigned int  BMS_Charge_A = 1000;   //充电需求电流
unsigned char Storage_Battery;       //蓄电池电压
unsigned char MCU_Control_Temp = 40; //电机控制器温度

//整车控制器3 ID：0X0CFA001D
//unsigned char VCU_Speed;             //整车发送车速
//unsigned char Three_Temp = 40;       //三合一温度
//unsigned int  Air_RPM;               //气泵转速
//unsigned int  Switch_RPM;            //转向泵转速
...
unsigned char BAT_Cell_Voltage[20];           //电池模块电压
unsigned char BAT_Cell_Temperature[20];       //电池模块温度
unsigned char BAT_Cell_Current[20];           //电池模块电流
DATA_BIT      BAT_Fault[20];                  //电池模块报警 

//吊具控制器  ID:0x18FF020A
DATA_BIT      DJ_Status[8];                   //吊具状态
...
void __attribute__((__interrupt__, auto_psv)) _C2Interrupt(void) {
    ...
    if (C2RX0CONbits.RXFUL) { //数据有效
        ...
        switch (msg.id) {  
            case 0x1000EF1D:
                VCU_Fault[0].byte = msg.data[0];
                Charging = (msg.data[0] >> 6) & 0x03;
                BMS_SOC = msg.data[1];
                MCU_RPM = ((unsigned char) msg.data[2]
                          +((unsigned int) msg.data[3] << 8));
                BMS_V = ((unsigned char) msg.data[4]
                        +((unsigned int) msg.data[5] << 8));
                BMS_A = ((unsigned char) msg.data[6]
                        +((unsigned int) msg.data[7] << 8));
                break;
                
            case 0x1100EF1D:
                VCU_Fault[1].byte = msg.data[0];
                VCU_Fault[2].byte = msg.data[1];
                BMS_Charge_V = ((unsigned char) msg.data[2]
                          +((unsigned int) msg.data[3] << 8));
                BMS_Charge_A = ((unsigned char) msg.data[4]
                        +((unsigned int) msg.data[5] << 8));
                Storage_Battery = msg.data[6];
                MCU_Control_Temp = msg.data[7];
                break;
             
            /*case 0x0CFA001D:
                Storage_Battery = msg.data[0];
                VCU_Speed = msg.data[1];
                MCU_Control_Temp = msg.data[2];
                Three_Temp = msg.data[3];
                Air_RPM = ((unsigned char) msg.data[4]
                        +((unsigned int) msg.data[5] << 8));
                Switch_RPM = ((unsigned char) msg.data[6]
                        +((unsigned int) msg.data[7] << 8));
                break;*/    
                
            case 0x0CFB001D:
                BAT_Max_Cell_Voltage = msg.data[0];
                BAT_Min_Cell_Voltage = msg.data[1];
                BAT_Max_Cell_Voltage_Cell_Case = msg.data[2];
                BAT_Min_Cell_Voltage_Cell_Case = msg.data[3];
                BAT_Max_Cell_Temperature = msg.data[4];
                BAT_Min_Cell_Temperature = msg.data[5];
                BAT_Max_Cell_Temp_Cell_Case = msg.data[6];
                BAT_Min_Cell_Temp_Cell_Case = msg.data[7];
                break;
            case 0x0CFD001D:      
                SPEED=((unsigned char) msg.data[0]+((unsigned int) msg.data[1] << 8))/10;//车速
                QY2= ((unsigned char) msg.data[2]+((unsigned int) msg.data[3] << 8));
                QY1= ((unsigned char) msg.data[4]+((unsigned int) msg.data[5] << 8));
                break;       
            case 0x18FF020A:
                for (i = 0; i < 8; i++) {
                    DJ_Status[i].byte = msg.data[i];
                }
                break;
#if 0           
            case 0x18F201F3:
        ...
        for (i = 0; i < 10; i++) {
            if (msg.id == (0x0CFC011D + ((unsigned int) i << 8))) {
                BAT_Cell_Voltage[i * 2] = msg.data[0];
                BAT_Cell_Temperature[i * 2] = msg.data[2];
                BAT_Cell_Current[i * 2] = msg.data[1];
                BAT_Fault[i * 2].byte = msg.data[3];
                BAT_Cell_Voltage[i * 2 + 1] = msg.data[4];
                BAT_Cell_Temperature[i * 2 + 1] = msg.data[6];
                BAT_Cell_Current[i * 2 + 1] = msg.data[5];
                BAT_Fault[i * 2 + 1].byte = msg.data[7];
            }
        }
